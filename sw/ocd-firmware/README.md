# ASTRARV32 On-Chip Debugger (OCD) - "Park Loop" Code (Firmware)
...
* processing and acknowledging execute requests
...
the debug memory (DM) module to check for requests.

> [!IMPORTANT]
> Executing `make clean image` will **NOT** update the actual debugger code ROM that
gets synthesized. The interface with the DM will break if there are any bugs in this code!
However, if you wish to update the code ROM, copy the array content from `park_loop_image.vhd`
to the `code_rom_c` constant in `rtl/core/astrarv32_debug_dm.vhd`.
