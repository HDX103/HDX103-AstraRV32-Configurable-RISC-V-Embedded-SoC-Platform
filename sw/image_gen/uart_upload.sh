...
# Simple script to upload an executable to the default ASTRARV32 bootloader
...
if [ $# -ne 2 ]
then
  printf "Upload an executable via serial port (UART) to the ASTRARV32 bootloader.\n"
  ...
  printf "Usage:   ./uart_upload.sh <serial port> <ASTRARV32 executable>\n"
  printf "Example: ./uart_upload.sh /dev/ttyS6 path/to/astrarv32_exe.bin\n"
  exit 0
fi
