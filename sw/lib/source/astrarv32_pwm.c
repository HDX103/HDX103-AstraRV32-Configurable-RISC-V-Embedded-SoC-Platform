// ================================================================================ //
// The ASTRARV32 RISC-V Processor - https://github.com/stnolting/astrarv32              //
// Copyright (c) ASTRARV32 contributors.                                              //
// Copyright (c) 2020 - 2026 Stephan Nolting. All rights reserved.                  //
// Licensed under the BSD-3-Clause license, see LICENSE for details.                //
// SPDX-License-Identifier: BSD-3-Clause                                            //
// ================================================================================ //

/**
 * @file astrarv32_pwm.c
 * @brief Pulse-Width Modulation Controller (PWM) HW driver source file.
 */

#include <astrarv32.h>


/**********************************************************************//**
 * Check if PWM unit was synthesized.
 *
 * @return 0 if PWM was not synthesized, non-zero if PWM is available.
 **************************************************************************/
int astrarv32_pwm_available(void) {

  return(int)(ASTRARV32_SYSINFO->SOC & (1 << SYSINFO_SOC_IO_PWM));
}


/**********************************************************************//**
 * Get number of implemented PWM channels.
 *
 * @return Number of implemented PWM channels (0..32).
 **************************************************************************/
int astrarv32_pwm_get_num_channels(void) {

  uint32_t backup = ASTRARV32_PWM->POLARITY;
  ASTRARV32_PWM->POLARITY = -1;
  uint32_t tmp = ASTRARV32_PWM->POLARITY;

  uint32_t i = 0, cnt = 0;
  for (i=0; i<16; i++) {
    cnt += tmp & 1;
    tmp >>= 1;
  }

  ASTRARV32_PWM->POLARITY = backup;

  return (int)cnt;
}


/**********************************************************************//**
 * Set global PWM counter clock prescaler.
 *
 * @param[in] prsc Clock prescaler select (0..7). See #ASTRARV32_CLOCK_PRSC_enum.
 **************************************************************************/
void astrarv32_pwm_set_clock(int prsc) {

  ASTRARV32_PWM->CLKPRSC = prsc;
}


/**********************************************************************//**
 * Enable PWM channel using bit mask.
 *
 * @param[in] mask Channel bit mask.
 **************************************************************************/
void astrarv32_pwm_ch_enable_mask(uint32_t mask) {

  __MMREG32_BSET(ASTRARV32_PWM->ENABLE, mask);
}


/**********************************************************************//**
 * Disable PWM channel using bit mask.
 *
 * @param[in] mask Channel bit mask.
 **************************************************************************/
void astrarv32_pwm_ch_disable_mask(uint32_t mask) {

  __MMREG32_BCLR(ASTRARV32_PWM->ENABLE, mask);
}


/**********************************************************************//**
 * Enable individual PWM channel.
 *
 * @param[in] ch Channel select (0..31).
 **************************************************************************/
void astrarv32_pwm_ch_enable_single(int ch) {

  __MMREG32_BSET(ASTRARV32_PWM->ENABLE, 1 << (ch & 31u));
}


/**********************************************************************//**
 * Disable individual PWM channel using bit mask.
 *
 * @param[in] ch Channel select (0..31).
 **************************************************************************/
void astrarv32_pwm_ch_disable_single(int ch) {

  __MMREG32_BCLR(ASTRARV32_PWM->ENABLE, 1 << (ch & 31u));
}


/**********************************************************************//**
 * Configure a single channel's wrap value and polarity.
 *
 * @param[in] ch Channel select (0..31).
 * @param[in] top Wrap value for PWM counter (16-bit).
 * @param[in] pol Polarity of PWM output (0 = normal, 1 = inverse).
 * @param[in] mode Operation mode (0 = fast-PWM, 1 = phase-correct PWM).
 **************************************************************************/
void astrarv32_pwm_ch_setup(int ch, int top, int pol, int mode) {

  ch &= 31u;
  uint32_t mask = 1 << ch;

  if (pol) {
    __MMREG32_BSET(ASTRARV32_PWM->POLARITY, mask);
  } else {
    __MMREG32_BCLR(ASTRARV32_PWM->POLARITY, mask);
  }

  if (mode) {
    __MMREG32_BSET(ASTRARV32_PWM->MODE, mask);
  } else {
    __MMREG32_BCLR(ASTRARV32_PWM->MODE, mask);
  }

  ASTRARV32_PWM->CHANNEL[ch].TOP = top;
}


/**********************************************************************//**
 * Set PWM channel's duty cycle.
 *
 * @param[in] ch Channel select (0..31).
 * @param[in] duty Duty cycle (16-bit).
 **************************************************************************/
void astrarv32_pwm_ch_set_duty(int ch, int duty) {

  ASTRARV32_PWM->CHANNEL[ch & 31u].CMP = duty;
}
