 ...
 **************************************************************************/
int astrarv32_smc_available(void) {
  return (int)(ASTRARV32_SYSINFO->SOC & (1 << SYSINFO_SOC_SMC));
}
 ...
 * @param[in] msize Memory chip size select (#ASTRARV32_SMC_MSIZE_enum).
 ...
 **************************************************************************/
void astrarv32_smc_setup(int dual, int msize, int cdiv, int rwait, uint8_t rcmd, uint8_t wcmd, uint32_t icmd) {

  // reset and disable
  ASTRARV32_SMC->CSR0 = 0;

  // set initialization commands
  ASTRARV32_SMC->CSR1 = icmd << SMC_CSR1_ICMD0_LSB;
  ...
  ASTRARV32_SMC->CSR0 = tmp;

  // wait for initialization sequence to complete
  while (astrarv32_smc_busy());
}
 ...
 **************************************************************************/
int astrarv32_smc_busy(void) {
  return (int)(ASTRARV32_SMC->CSR0 & (1<<SMC_CSR0_BUSY));
}

/**********************************************************************//**
 * Enable SMC control of SMC IO pins.
 **************************************************************************/
void astrarv32_smc_pins_enable(void) {
  __MMREG32_BSET(ASTRARV32_SMC->CSR0, 1<<SMC_CSR0_IOEN);
}

/**********************************************************************//**
 * Disable SMC control of SMC IO pins.
 **************************************************************************/
void astrarv32_smc_pins_disable(void) {
  __MMREG32_BCLR(ASTRARV32_SMC->CSR0, 1<<SMC_CSR0_IOEN);
}
 ...
 **************************************************************************/
uint32_t astrarv32_smc_get_clockspeed(void) {
  uint32_t f_clk = astrarv32_sysinfo_get_clk();
  uint32_t cdiv  = (ASTRARV32_SMC->CSR0 >> SMC_CSR0_CDIV_LSB) & 0x07U;
 ...
 **************************************************************************/
uint32_t astrarv32_smc_get_baseaddr(void) {
  uint32_t tmp = (ASTRARV32_SMC->CSR1 >> SMC_CSR1_MEM_BASE_LSB);
