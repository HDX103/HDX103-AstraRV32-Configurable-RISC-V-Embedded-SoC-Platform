// ================================================================================ //
// The ASTRARV32 RISC-V Processor - https://github.com/stnolting/astrarv32              //
// Copyright (c) ASTRARV32 contributors.                                              //
// Copyright (c) 2020 - 2026 Stephan Nolting. All rights reserved.                  //
// Licensed under the BSD-3-Clause license, see LICENSE for details.                //
// SPDX-License-Identifier: BSD-3-Clause                                            //
// ================================================================================ //

/**
 * @file astrarv32_newlib.c
 * @brief ASTRARV32-specific Newlib system calls
 * @note Sources:
 * https://www.sourceware.org/newlib/libc.html#Syscalls
 * https://interrupt.memfault.com/blog/boostrapping-libc-with-newlib
 * https://www.embecosm.com/appnotes/ean9/ean9-howto-newlib-1.0.pdf
 */

#include <astrarv32.h>
#include <newlib.h>
#include <time.h>
#include <unistd.h>
#include <sys/stat.h>
#include <sys/time.h>

// global error variable
#include <errno.h>
#undef errno
extern int errno;

// attributes to force syscall symbols (newlib link-time-optimization issue)
#define SYSCALL_ATTR __attribute__((used, externally_visible, noinline))

// global environment
char *__env[1] = { 0 };
char **environ = __env;


/**********************************************************************//**
 * Issue a warning when semihosting is enabled.
 **************************************************************************/
/**@{*/
#ifdef STDIO_SEMIHOSTING
  #warning Newlib/stdio.h semihosting enabled.
#endif
#ifdef TIME_SEMIHOSTING
  #warning Newlib/time.h semihosting enabled.
#endif
/**@}*/


 /**********************************************************************//**
 * Exit a program without cleaning up anything.
 **************************************************************************/
SYSCALL_ATTR void _exit(int status) {

  // put status into register 'a0' and jump to crt0's exit code
  asm volatile (
    ".extern __crt0_main_exit \n"
    "la ra,  __crt0_main_exit \n"
    "mv a0,  %[exit_code]     \n"
    "jr ra                    \n"
    : : [exit_code] "r" (status)
  );

  // will never be reached
  __builtin_unreachable();
  while(1);
}


 /**********************************************************************//**
 * Open file handle.
 **************************************************************************/
SYSCALL_ATTR int _open(char *pathname, int flags) {
  (void)pathname;
  (void)flags;
#ifdef STDIO_SEMIHOSTING
  return astrarv32_semihosting_open(pathname, flags);
#else
  return -1; // no files available
#endif
}


 /**********************************************************************//**
 * Close file handle.
 **************************************************************************/
SYSCALL_ATTR int _close(int file) {
  (void)file;
#ifdef STDIO_SEMIHOSTING
  return astrarv32_semihosting_close(file);
#else
  return -1; // no files available
#endif
}


 /**********************************************************************//**
 * Status of an open file. All files are regarded as character special devices.
 **************************************************************************/
SYSCALL_ATTR int _fstat(int file, struct stat *st) {
  (void)file;
  st->st_mode = S_IFCHR;
  return 0;
}


 /**********************************************************************//**
 * Create new process.
 **************************************************************************/
SYSCALL_ATTR int _fork(void) {
  errno = EAGAIN;
  return -1;
}


 /**********************************************************************//**
 * Process-ID; this is sometimes used to generate strings unlikely to
 * conflict with other processes.
 **************************************************************************/
SYSCALL_ATTR int _getpid() {
  return 1; // there is only one process by default
}


 /**********************************************************************//**
 * Query whether output stream is a terminal.
 **************************************************************************/
SYSCALL_ATTR int _isatty(int file) {
  (void)file;
#ifdef STDIO_SEMIHOSTING
  return astrarv32_semihosting_istty(file);
#else
  return 1; // all streams are terminals
#endif
}


 /**********************************************************************//**
 * Send a signal.
 **************************************************************************/
SYSCALL_ATTR int _kill(int pid, int sig) {
  (void)pid;
  (void)sig;
  errno = EINVAL;
  return -1;
}


 /**********************************************************************//**
 * Rename existing file.
 **************************************************************************/
SYSCALL_ATTR int _link(char *old_name, char *new_name) {
  (void)old_name;
  (void)new_name;
  errno = EMLINK;
  return -1;
}


 /**********************************************************************//**
 * Set position in a file.
 **************************************************************************/
SYSCALL_ATTR int _lseek(int file, int ptr, int dir) {
  (void)file;
  (void)ptr;
  (void)dir;
#ifdef STDIO_SEMIHOSTING
  return astrarv32_semihosting_seek(file, ptr);
#else
  return 0;
#endif
}


 /**********************************************************************//**
 * Status of a file.
 **************************************************************************/
SYSCALL_ATTR int _stat(char *file, struct stat *st) {
  (void)file;
  st->st_mode = S_IFCHR; // all files are character special devices
  return 0;
}


 /**********************************************************************//**
 * Wait for child process.
 **************************************************************************/
SYSCALL_ATTR int _wait(int status) {
  (void)status;
  errno = ECHILD;
  return -1;
}


 /**********************************************************************//**
 * Remove a file's directory entry.
 **************************************************************************/
SYSCALL_ATTR int _unlink(char *name) {
  (void)name;
  errno = ENOENT;
  return -1;
}


 /**********************************************************************//**
 * Read from a file. STDIN will read from UART0, all other input streams
 * will read from UART1.
 **************************************************************************/
SYSCALL_ATTR int _read(int file, char *ptr, int len) {

#ifdef STDIO_SEMIHOSTING
  return astrarv32_semihosting_read(file, ptr, len);
#else
  char c = 0;
  int read_cnt = 0;

  // read STDIN stream from ASTRARV32.UART0 (if available)
  if ((file == STDIN_FILENO) && (astrarv32_uart_available(ASTRARV32_UART0))) {
    while (len--) {
      c = (char)astrarv32_uart_getc(ASTRARV32_UART0);
      *ptr++ = c;
      read_cnt++;
      if ((c == '\n') || (c == '\r')) { // also terminate on [press enter]
        return read_cnt;
      }
    }
    return read_cnt;
  }
  // read all other input streams from ASTRARV32.UART1 (if available)
  else if (astrarv32_uart_available(ASTRARV32_UART1)) {
    while (len--) {
      c = (char)astrarv32_uart_getc(ASTRARV32_UART1);
      *ptr++ = c;
      read_cnt++;
      if ((c == '\n') || (c == '\r')) { // also terminate on [press enter]
        return read_cnt;
      }
    }
    return read_cnt;
  }
  else {
    errno = ENOSYS;
    return -1;
  }
#endif
}


 /**********************************************************************//**
 * Write to a file. STDOUT and STDERR will write to UART0, all other
 * output streams will write to UART1.
 **************************************************************************/
SYSCALL_ATTR int _write(int file, char *ptr, int len) {

#ifdef STDIO_SEMIHOSTING
  return astrarv32_semihosting_write(file, ptr, len);
#else
  int write_cnt = 0;

  // write STDOUT and STDERR streams to ASTRARV32.UART0 (if available)
  if ((file == STDOUT_FILENO) || (file == STDERR_FILENO)) {
    if (astrarv32_uart_available(ASTRARV32_UART0)) {
      while (len--) {
        astrarv32_uart_putc(ASTRARV32_UART0, *ptr++);
        write_cnt++;
      }
      return write_cnt;
    }
    else {
      errno = ENOSYS;
      return -1;
    }
  }

  // write all other output streams to ASTRARV32.UART1 (if available)
  if (astrarv32_uart_available(ASTRARV32_UART1)) {
    while (len--) {
      astrarv32_uart_putc(ASTRARV32_UART1, *ptr++);
      write_cnt++;
    }
    return write_cnt;
  }
  else {
    errno = ENOSYS;
    return -1;
  }
#endif
}


 /**********************************************************************//**
 * Dynamic memory management. Used by "malloc" and "free", among others.
 **************************************************************************/
SYSCALL_ATTR void *_sbrk(int incr) {

  static unsigned char *curr_heap = NULL; // current heap pointer
  unsigned char *prev_heap; // previous heap pointer

  // initialize
  if (curr_heap == NULL) {
    curr_heap = (unsigned char *)ASTRARV32_HEAP_BEGIN;
  }

  // do we have a heap at all?
  if ((ASTRARV32_HEAP_BEGIN == ASTRARV32_HEAP_END) || (ASTRARV32_HEAP_SIZE == 0)) {
#ifdef NEWLIB_DEBUG
    write(STDERR_FILENO, "[astrarv32-newlib] no heap available\r\n", 36);
#endif
    errno = ENOMEM;
    return (void*)-1;
  }

  // sufficient space left?
  if ((uintptr_t)(curr_heap + incr) >= ASTRARV32_HEAP_END) {
#ifdef NEWLIB_DEBUG
    write(STDERR_FILENO, "[astrarv32-newlib] heap exhausted\r\n", 33);
#endif
    errno = ENOMEM;
    return (void*)-1;
  }

  // underrun?
  if ((incr < 0) && ((uintptr_t)(curr_heap + incr) < ASTRARV32_HEAP_BEGIN)) {
#ifdef NEWLIB_DEBUG
    write(STDERR_FILENO, "[astrarv32-newlib] heap underrun\r\n", 32);
#endif
    errno = ENOMEM;
    return (void*)-1;
  }

  prev_heap = curr_heap;
  curr_heap += incr;

  return (void*)prev_heap;
}


 /**********************************************************************//**
 * Get Unix time. Used by "time", among others.
 **************************************************************************/
SYSCALL_ATTR int _gettimeofday(struct timeval *tv) {
#ifdef TIME_SEMIHOSTING
  tv->tv_sec  = (time_t)astrarv32_semihosting_time();
  tv->tv_usec = 0;
  return 0;
#else
  // use MTIME as system time (if available)
  if (astrarv32_clint_available()) {
    tv->tv_sec  = (time_t)astrarv32_clint_unixtime_get();
    tv->tv_usec = 0;
    return 0;
  }
  else {
    errno = ENOSYS;
    return -1;
  }
#endif
}
