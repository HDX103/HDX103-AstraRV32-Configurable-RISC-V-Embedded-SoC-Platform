// ================================================================================ //
// The ASTRARV32 RISC-V Processor - https://github.com/stnolting/astrarv32              //
// Copyright (c) ASTRARV32 contributors.                                              //
// Copyright (c) 2020 - 2026 Stephan Nolting. All rights reserved.                  //
// Licensed under the BSD-3-Clause license, see LICENSE for details.                //
// SPDX-License-Identifier: BSD-3-Clause                                            //
// ================================================================================ //

/**
 * @file astrarv32_gpio.c
 * @brief General purpose input/output port unit (GPIO) HW driver source file.
 */

#include <astrarv32.h>


/**********************************************************************//**
 * Check if GPIO unit was synthesized.
 *
 * @return 0 if GPIO was not synthesized, non-zero if GPIO is available.
 **************************************************************************/
int astrarv32_gpio_available(void) {

  return (int)(ASTRARV32_SYSINFO->SOC & (1 << SYSINFO_SOC_IO_GPIO));
}


/**********************************************************************//**
 * Set single pin of GPIO's output port.
 *
 * @param[in] pin Output pin number to be set (0..31).
 * @param[in] value Set pint high (1) or low (0).
 **************************************************************************/
void astrarv32_gpio_pin_set(int pin, int value) {

  uint32_t mask = (uint32_t)(1 << (pin & 31u));

  if (value) {
    __MMREG32_BSET(ASTRARV32_GPIO->PORT_OUT, mask);
  }
  else {
    __MMREG32_BCLR(ASTRARV32_GPIO->PORT_OUT, mask);
  }
}


/**********************************************************************//**
 * Toggle single pin of GPIO's output port.
 *
 * @param[in] pin Output pin number to be toggled (0..31).
 **************************************************************************/
void astrarv32_gpio_pin_toggle(int pin) {

  __MMREG32_BINV(ASTRARV32_GPIO->PORT_OUT, 1 << (pin & 31u));
}


/**********************************************************************//**
 * Get single pin of GPIO's input port.
 *
 * @param[in] pin Input pin to be read (0..31).
 * @return zero if pin is low, non-zero if pin is high.
 **************************************************************************/
uint32_t astrarv32_gpio_pin_get(int pin) {

  return ASTRARV32_GPIO->PORT_IN & (uint32_t)(1 << (pin & 31u));
}


/**********************************************************************//**
 * Set complete GPIO output port.
 *
 * @param[in] pin_mask New output port value (32-bit).
 **************************************************************************/
void astrarv32_gpio_port_set(uint32_t pin_mask) {

  ASTRARV32_GPIO->PORT_OUT = pin_mask;
}


/**********************************************************************//**
 * Toggle bit in entire GPIO output port.
 *
 * @param[in] pin_mask Bit mask; set bits will toggle the according output pins (32-bit).
 **************************************************************************/
void astrarv32_gpio_port_toggle(uint32_t pin_mask) {

  __MMREG32_BINV(ASTRARV32_GPIO->PORT_OUT, pin_mask);
}


/**********************************************************************//**
 * Get complete GPIO input port.
 *
 * @return Current input port state (32-bit).
 **************************************************************************/
uint32_t astrarv32_gpio_port_get(void) {

  return ASTRARV32_GPIO->PORT_IN;
}


/**********************************************************************//**
 * Set direction of GPIO port.
 * @note Direction control must be enabled by the according top generic.
 *
 * @param[in] pin_mask Direction port configuration (32-bit), one bit per port: 0 = input, 1 = output.
 **************************************************************************/
void astrarv32_gpio_dir_set(uint32_t pin_mask) {

  ASTRARV32_GPIO->PORT_DIR = pin_mask;
}


/**********************************************************************//**
 * Get direction of GPIO port.
 * @note Direction control must be enabled by the according top generic.
 *
 * @return Current direction port state (32-bit), one bit per port: 0 = input, 1 = output.
 **************************************************************************/
uint32_t astrarv32_gpio_dir_get(void) {

  return ASTRARV32_GPIO->PORT_DIR;
}


/**********************************************************************//**
 * Configure pin interrupt trigger.
 *
 * @param[in] pin Input pin select (0..31).
 * @param[in] trigger Trigger select (#GPIO_TRIGGER_enum).
 **************************************************************************/
void astrarv32_gpio_irq_setup(int pin, int trigger) {

  uint32_t mask = (uint32_t)(1 << (pin & 31u));

  // trigger type
  if ((trigger == GPIO_TRIG_EDGE_FALLING) || (trigger == GPIO_TRIG_EDGE_RISING)) {
    __MMREG32_BSET(ASTRARV32_GPIO->IRQ_TYPE, mask); // set = edge
  }
  else {
    __MMREG32_BCLR(ASTRARV32_GPIO->IRQ_TYPE, mask); // clear = level
  }

  // polarity type
  if ((trigger == GPIO_TRIG_EDGE_RISING) || (trigger == GPIO_TRIG_LEVEL_HIGH)) {
    __MMREG32_BSET(ASTRARV32_GPIO->IRQ_POLARITY, mask); // set = rising edge / high level
  }
  else {
    __MMREG32_BCLR(ASTRARV32_GPIO->IRQ_POLARITY, mask); // clear = falling edge / low level
  }
}


/**********************************************************************//**
 * Enable input pin interrupt(s).
 *
 * @param[in] pin_mask Pin-IRQ enable mask (set to 1 to enable the according pin).
 **************************************************************************/
void astrarv32_gpio_irq_enable(uint32_t pin_mask) {

  __MMREG32_BSET(ASTRARV32_GPIO->IRQ_ENABLE, pin_mask);
}


/**********************************************************************//**
 * Disable input pin interrupt(s).
 *
 * @param[in] pin_mask Pin-IRQ enable mask (set to 1 to disable the according pin).
 **************************************************************************/
void astrarv32_gpio_irq_disable(uint32_t pin_mask) {

  __MMREG32_BCLR(ASTRARV32_GPIO->IRQ_ENABLE, pin_mask);
}


/**********************************************************************//**
 * Get currently pending GPIO input interrupts.
 *
 * @param[in] Pending inputs (bit mask; high = pending).
 **************************************************************************/
uint32_t astrarv32_gpio_irq_get(void) {

  return ASTRARV32_GPIO->IRQ_PENDING;
}


/**********************************************************************//**
 * Clear pending GPIO input interrupts via bit mask.
 *
 * @param[in] clr_mask Clear mask (bit high = clear according pending interrupt).
 **************************************************************************/
void astrarv32_gpio_irq_clr(uint32_t clr_mask) {

  __MMREG32_BCLR(ASTRARV32_GPIO->IRQ_PENDING, clr_mask);
}
