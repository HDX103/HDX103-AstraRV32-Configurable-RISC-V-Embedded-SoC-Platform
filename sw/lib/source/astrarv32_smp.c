 ...
 **************************************************************************/
static int __astrarv32_smp_boot(uint32_t addr, uint32_t stack) {

  // setup launch configuration in CLINT.MTIMECMP[hart_id]
  ASTRARV32_CLINT->MTIMECMP[1].uint32[0] = stack; // top of stack
  ASTRARV32_CLINT->MTIMECMP[1].uint32[1] = addr;  // entry point

  // start core by triggering its software interrupt
  astrarv32_clint_msi_set(1);

  // wait for core acknowledge
  uint32_t timeout = astrarv32_sysinfo_get_clk() >> 10; // enough time for cache misses and bus latency
  while (timeout--) {
    if (astrarv32_clint_msi_get(1) == 0) {
      return 0;
    }
  }
...
int astrarv32_smp_launch(int (*entry_point)(void), uint8_t* stack_memory, size_t stack_size_bytes) {
  ...
  if ((astrarv32_smp_whoami() != 0) || // this can be executed on core0 only
      (astrarv32_sysinfo_get_numcores() < 2) || // core not available
  ...
  if (__astrarv32_smp_boot((uint32_t)__crt0_entry, stack_top)) {
  ...
  if (__astrarv32_smp_boot((uint32_t)entry_point, stack_top)) {
    return -2; // core did not respond
  }
