...
void gpio_interrupt_handler(void) {
  ...
  uint32_t tmp = astrarv32_gpio_irq_get();

  // clear all pending GPIO pin interrupts
  astrarv32_gpio_irq_clr(-1);

  // show all currently pending GPIO input interrupts
  astrarv32_uart0_printf("\n[IRQ] ");
  astrarv32_uart0_printf("triggering pins = 0x%x, ", tmp);

  // show current GPIO input state
  astrarv32_uart0_printf("GPIO.input = 0x%x", astrarv32_gpio_port_get());
}
...
int main(void) {
  ...
  // setup ASTRARV32 runtime-environment (RTE)
  astrarv32_rte_setup();
  ...
  astrarv32_uart0_printf("\n<< ASTRARV32 GPIO IRQ Demo >>\n\n");

  // check hardware/software configuration
  if (astrarv32_gpio_available() == 0) { // GPIO available?
    astrarv32_uart0_printf("[ERROR] GPIO module not available!\n");
    return -1;
  }

  // clear output port
  astrarv32_gpio_port_set(0x00000000);

  // configure all GPIO pins as input; only relevant if the GPIO direction control feature is enabled
  astrarv32_gpio_dir_set(0);

  // configure CPU's GPIO controller interrupt
  astrarv32_rte_handler_install(GPIO_TRAP_CODE, gpio_interrupt_handler); // install GPIO trap handler
  astrarv32_cpu_csr_set(CSR_MIE, 1 << GPIO_FIRQ_ENABLE); // enable GPIO FIRQ channel
  astrarv32_cpu_csr_set(CSR_MSTATUS, 1 << CSR_MSTATUS_MIE); // enable machine-mode interrupts

  // configure GPIO input's IRQ trigger
  astrarv32_gpio_irq_setup(0, GPIO_TRIG_EDGE_RISING);  // input pin 0: interrupt on a rising edge
  astrarv32_gpio_irq_setup(1, GPIO_TRIG_EDGE_FALLING); // input pin 1: interrupt on a falling edge
  astrarv32_gpio_irq_setup(2, GPIO_TRIG_LEVEL_HIGH);   // input pin 2: interrupt on high-level
//astrarv32_gpio_irq_setup(3, GPIO_TRIG_LEVEL_LOW);    // input pin 3: interrupt on low-level

  // enable GPIO input pin interrupts; argument is an "enable bit mask" - one bit for each input pin
  astrarv32_gpio_irq_enable((1<<0) | (1<<1) | (1<<2));
