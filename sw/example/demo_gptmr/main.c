...
void gptmr_firq_handler(void) {
  ...
  int irq_src = astrarv32_gptmr_irq_get(); // get pending slice IRQ with highest priority
  astrarv32_gptmr_irq_ack(irq_src); // acknowledge (clear) slice interrupt
  astrarv32_uart0_printf("IRQ from slice %u\n", irq_src);
}
...
int main() {
  ...
  if (astrarv32_gptmr_available() == 0) {
    astrarv32_uart0_printf("ERROR! GPTMR not implemented!\n");
    return 1;
  }

  // intro
  astrarv32_uart0_printf("General purpose timer (GPTMR) demo Program.\n\n");

  // get number of implemented timer slice
  int num_slices = astrarv32_gptmr_get_num_slices();
  astrarv32_uart0_printf("Number GPTMR slices: %u\n", num_slices);

  // setup GPTMR, install interrupt handler and enable interrupt
  astrarv32_gptmr_setup(CLK_PRSC_8);
  astrarv32_rte_handler_install(GPTMR_TRAP_CODE, gptmr_firq_handler);
  astrarv32_cpu_csr_set(CSR_MIE, 1 << GPTMR_FIRQ_ENABLE);   // enable GPTMR FIRQ channel
  ...
  int i;
  uint32_t t_base = (astrarv32_sysinfo_get_clk() / 8) / 2;
  for (i=0; i<num_slices; i++){
    astrarv32_gptmr_configure(i, 0, (i+1)*t_base, 1);
  }

  // enable all available slices
  astrarv32_gptmr_enable_mask((1 << num_slices) - 1);
