...
/**********************************************************************//**
 * Simple demo program to showcase the ASTRARV32 DMA controller.
 *
...
int main() {
  ...
  if (astrarv32_dma_available() == 0) {
    astrarv32_uart0_printf("ERROR! DMA controller not implemented!\n");
    return 1;
  }

  // show base address of test data arrays and DMA's FIFO size
  astrarv32_uart0_printf("Source test data:      %u bytes @ 0x%x\n", (uint32_t)(sizeof(dma_src)), (uint32_t)(&dma_src[0]));
  astrarv32_uart0_printf("Destination test data: %u bytes @ 0x%x\n", (uint32_t)(sizeof(dma_src)), (uint32_t)(&dma_dst[0]));
  astrarv32_uart0_printf("Descriptor FIFO depth: %u\n", astrarv32_dma_get_descriptor_fifo_depth());

  // install DMA interrupt handler
  astrarv32_rte_handler_install(DMA_TRAP_CODE, dma_firq_handler);
  ...
  if (dma_rc) {
    astrarv32_uart0_printf("Programming DMA descriptor failed!\n");
    return -1;
  }
  ...
  astrarv32_uart0_printf("Waiting for DMA... ");
  while (1) {
    dma_rc = astrarv32_dma_status();
    if (dma_rc == DMA_STATUS_DONE) {
      astrarv32_uart0_printf("Transfer done.\n");
      break;
    }
    else if (dma_rc == DMA_STATUS_ERROR) {
      astrarv32_uart0_printf("Transfer failed!\n");
      break;
    }
  }
  ...
  if ((dma_dst[0] != 0x00112233) ||
    ...
    astrarv32_uart0_printf("Incorrect DST data!\n");
  }
  else {
    astrarv32_uart0_printf("Transfer succeeded!\n");
  }
  show_arrays();
  ...
  dma_rc = astrarv32_dma_program(
    (uint32_t)(&dma_src[0]),     // source array base address - byte-aligned
    (uint32_t)(ASTRARV32_DM_BASE), // destination base address - byte-aligned
    DMA_SRC_INC_BYTE |           // read source data as incrementing bytes
    DMA_DST_INC_WORD |           // write destination data as incrementing words
    DMA_BSWAP        |           // swap byte order
    4                            // number of elements to transfer: 4
  ...
  astrarv32_dma_irq_ack();

  // configure DMA interrupt
  astrarv32_cpu_csr_set(CSR_MIE, 1 << DMA_FIRQ_ENABLE); // enable DMA interrupt source
  ...
  // check if transfer was successful
  if ((astrarv32_dma_status() != DMA_STATUS_IDLE) || // DMA is in idle mode without errors
...
void show_arrays(void) {
  ...
  astrarv32_uart0_printf("---------------------------\n");
  astrarv32_uart0_printf("     SRC         DST\n");
  astrarv32_uart0_printf("[0]  0x%x  0x%x\n", dma_src[0], dma_dst[0]);
  astrarv32_uart0_printf("[1]  0x%x  0x%x\n", dma_src[1], dma_dst[1]);
  astrarv32_uart0_printf("[2]  0x%x  0x%x\n", dma_src[2], dma_dst[2]);
  astrarv32_uart0_printf("[3]  0x%x  0x%x\n", dma_src[3], dma_dst[3]);
  astrarv32_uart0_printf("---------------------------\n");
}
...
void dma_firq_handler(void) {
  ...
  astrarv32_dma_irq_ack(); // clear DMA-done and DMA-error flags
  astrarv32_uart0_printf("<<DMA interrupt>>\n");
}
