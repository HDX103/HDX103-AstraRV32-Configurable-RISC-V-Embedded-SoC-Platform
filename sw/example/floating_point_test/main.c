 ...
 * @brief Verification program for the ASTRARV32 'Zfinx' extension (floating-point in x registers) using
...
#include <float.h>
#include <math.h>
#include "astrarv32_zfinx_extension_intrinsics.h"
...
/**********************************************************************//**
 * Main function; test all available operations of the ASTRARV32 'Zfinx'
...
int main() {
  ...
  if ((astrarv32_cpu_csr_read(CSR_MXISA) & (1<<CSR_MXISA_ZFINX)) == 0) {
    astrarv32_uart0_puts("Error! <Zfinx> extension not synthesized!\n");
    return 1;
  }


  // intro
  astrarv32_uart0_printf("ASTRARV32 Zfinx ISA extension (FPU) test\n\n");
#if (SILENT_MODE != 0)
  astrarv32_uart0_printf("SILENT_MODE enabled (only showing actual errors)\n");
#endif
  astrarv32_uart0_printf("Test cases per instruction: %u\n", (uint32_t)NUM_TEST_CASES);
  astrarv32_uart0_printf("NOTE: The ASTRARV32 FPU does not support subnormal numbers yet. Subnormal numbers are flushed to zero.\n\n");
...
// ----------------------------------------------------------------------------
  ...
  for (i=0;i<(uint32_t)NUM_TEST_CASES; i++) {
    ...
    astrarv32_cpu_csr_write(CSR_FFLAGS, opa.binary_value);
    res_hw.binary_value = astrarv32_cpu_csr_read(CSR_FFLAGS);
  ...
  for (i=0;i<(uint32_t)NUM_TEST_CASES; i++) {
    ...
    astrarv32_cpu_csr_write(CSR_FRM, opa.binary_value);
    res_hw.binary_value = astrarv32_cpu_csr_read(CSR_FRM);
    res_sw.binary_value = opa.binary_value;
    err_cnt += verify_result(i, opa.binary_value, 0, res_sw.binary_value, res_hw.binary_value);
  }
  print_report(err_cnt);
  ...
  for (i=0;i<(uint32_t)NUM_TEST_CASES; i++) {
    ...
    astrarv32_cpu_csr_write(CSR_FCSR, opa.binary_value);
    res_hw.binary_value = astrarv32_cpu_csr_read(CSR_FCSR);
    res_sw.binary_value = opa.binary_value;
    err_cnt += verify_result(i, opa.binary_value, 0, res_sw.binary_value, res_hw.binary_value);
  }
  print_report(err_cnt);
...
// ----------------------------------------------------------------------------
  ...
  err_cnt = 0;
  for (i=0;i<(uint32_t)NUM_TEST_CASES; i++) {
    astrarv32_cpu_csr_write(CSR_FFLAGS, 0);
    ...
    res_hw.binary_value = astrarv32_cpu_csr_read(CSR_FFLAGS) & (1 << CSR_FFLAGS_NV);
...
// ----------------------------------------------------------------------------
  astrarv32_cpu_csr_write(CSR_FCSR, 0); // clear exception flags and set "round to nearest"
...
// ----------------------------------------------------------------------------
  ...
  astrarv32_cpu_csr_write(CSR_MCAUSE, 0);
  ...
  if (astrarv32_cpu_csr_read(CSR_MCAUSE) == TRAP_CODE_I_ILLEGAL) {
    astrarv32_uart0_printf("%c[1m[FAILED]%c[0m\n", 27, 27);
    astrarv32_uart0_printf("Addition of 1.0 + (-1.0) timed out\n");
    err_cnt_total++;
  }
  print_report(err_cnt);
  test_cnt++;
  ...
  for (i=0;i<254; i++) {
    ...
    if (astrarv32_cpu_csr_read(CSR_MCAUSE) == TRAP_CODE_I_ILLEGAL) {
      astrarv32_uart0_printf("%c[1m[FAILED]%c[0m\n", 27, 27);
      astrarv32_uart0_printf("Conversion of opa with %d exponent timed out\n",opa.binary_value>>23);
      err_cnt_total++;
    }
    astrarv32_cpu_csr_write(CSR_MCAUSE, 0);
  ...
  for (i=0;i<253; i++) {
    ...
    if (astrarv32_cpu_csr_read(CSR_MCAUSE) == TRAP_CODE_I_ILLEGAL) {
      astrarv32_uart0_printf("%c[1m[FAILED]%c[0m\n", 27, 27);
      astrarv32_uart0_printf("Addition using opa with %d exponent and opb with %d exponent timed out\n",opa.binary_value>>23,opb.binary_value>>23);
      err_cnt_total++;
    }
    astrarv32_cpu_csr_write(CSR_MCAUSE, 0);
  ...
  for (i=0;i<253; i++) {
    ...
    if (astrarv32_cpu_csr_read(CSR_MCAUSE) == TRAP_CODE_I_ILLEGAL) {
      astrarv32_uart0_printf("%c[1m[FAILED]%c[0m\n", 27, 27);
      astrarv32_uart0_printf("Subtraction using opa with %d exponent and opb with %d exponent timed out\n",opa.binary_value>>23,opb.binary_value>>23);
      err_cnt_total++;
    }
    astrarv32_cpu_csr_write(CSR_MCAUSE, 0);
    // increment opb exponent by 1
...
// ----------------------------------------------------------------------------
  ...
  if (astrarv32_cpu_csr_read(CSR_MCAUSE) != TRAP_CODE_I_ILLEGAL) {
    ...
    astrarv32_uart0_printf("%c[1m[ok]%c[0m\n", 27, 27);
  }
...
// ----------------------------------------------------------------------------
  ...
  astrarv32_uart0_printf("FCVT.S.W: ");
  time_sw = 0;
  time_hw = 0;
  err_cnt = 0;
  for (i=0; i<num_runs; i++) {
    ...
    time_start = astrarv32_cpu_csr_read(CSR_CYCLE);
    {
      res_hw.float_value = riscv_intrinsic_fcvt_sw((int32_t)opa.binary_value);
    }
    time_hw += astrarv32_cpu_csr_read(CSR_CYCLE) - time_start;
    ...
    time_sw += astrarv32_cpu_csr_read(CSR_CYCLE) - time_start;
  ...
  if (err_cnt == 0) {
    astrarv32_uart0_printf("cycles[SW] = %u vs. cycles[HW] = %u\n", time_sw/num_runs, time_hw/num_runs);
  }
  else {
    astrarv32_uart0_printf("%c[1m[TEST FAILED!]%c[0m\n", 27, 27);
    err_cnt_total++;
  }


  // float to signed integer
  astrarv32_uart0_printf("FCVT.W.S: ");
  time_sw = 0;
  time_hw = 0;
  err_cnt = 0;
  for (i=0; i<num_runs; i++) {
  ...
  // Sign-injection
  astrarv32_uart0_printf("FSGNJX.S: ");
  time_sw = 0;
  time_hw = 0;
  err_cnt = 0;
  for (i=0; i<num_runs; i++) {
...
// ----------------------------------------------------------------------------
  ...
  if (err_cnt_total != 0) {
    ...
    astrarv32_uart0_printf("%u errors in %u test cases\n", err_cnt_total, test_cnt*(uint32_t)NUM_TEST_CASES);
    return 1;
  }
  else {
...
uint32_t get_test_vector(void) {
  ...
  if ((astrarv32_aux_xorshift32() & 0xff) == 0xff) {

    switch((astrarv32_aux_xorshift32() >> 10) & 0x3) { // random decision which special value we are taking
    ...
    tmp.binary_value = astrarv32_aux_xorshift32();
  }
...
#if (SILENT_MODE == 0)
  astrarv32_uart0_printf("%u: opa = 0x%x, opb = 0x%x : ref[SW] = 0x%x vs. res[HW] = 0x%x ", num, opa, opb, ref, res);
#endif
...
void print_report(uint32_t num_err) {
  ...
  astrarv32_uart0_printf("Errors: %u/%u ", num_err, (uint32_t)NUM_TEST_CASES);
