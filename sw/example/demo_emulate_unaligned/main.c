 ...
 * context-access functions of the ASTRARV32 run-time environment.
 **************************************************************************/
...
void trap_handler_emulate_unaligned_lw(void) {
  ...
  uint32_t inst = astrarv32_cpu_load_unsigned_word(astrarv32_cpu_csr_read(CSR_MEPC));
  ...
  if ((opcode == 0b0000011) && (funct3 == 0b010)) {
    ...
    uint32_t rs1 = astrarv32_rte_context_get(rs1_addr);

    // emulated function
    uint32_t addr = rs1 + imm12;
    uint32_t b0 = (uint32_t)astrarv32_cpu_load_unsigned_byte(addr + 0);
    uint32_t b1 = (uint32_t)astrarv32_cpu_load_unsigned_byte(addr + 1);
    uint32_t b2 = (uint32_t)astrarv32_cpu_load_unsigned_byte(addr + 2);
    uint32_t b3 = (uint32_t)astrarv32_cpu_load_unsigned_byte(addr + 3);
    uint32_t rd = (b3 << 24) | (b2 << 16) | (b1 << 8) | (b0 << 0);

    // write result back to main's context
    astrarv32_rte_context_put(rd_addr, rd);

  }
  else {
    astrarv32_uart0_printf("Unexpected instruction @ 0x%x!\n", astrarv32_cpu_csr_read(CSR_MEPC));
  }
}
...
int main() {
  ...
  astrarv32_uart0_printf("\nSource data:\n");
  astrarv32_uart0_printf("MEM[0x%x] = 0x%x\n", (uint32_t)&data_block[0], data_block[0]);
  ...
  astrarv32_uart0_printf("MEM[0x%x] = ", addr);

  // read from unaligned address
  data = astrarv32_cpu_load_unsigned_word(addr); // this will raise an exception

  if (data == 0x77001122) {
    astrarv32_uart0_printf("0x%x [ok]\n", data);
  }
  else {
  ...
  astrarv32_rte_handler_install(TRAP_CODE_L_MISALIGNED, trap_handler_emulate_unaligned_lw);
