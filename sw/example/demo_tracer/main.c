...
void tracer_irq_handler(void) {
  ...
  int i = 0;
  astrarv32_uart0_printf("Trace log:\n");
  while (astrarv32_tracer_data_avail()) {
    // read trace buffer entry
    delta_src = astrarv32_tracer_data_get_src();
    delta_dst = astrarv32_tracer_data_get_dst();
    // print branch source and destination (same format as the GDB output)
    astrarv32_uart0_printf("[%d] SRC: 0x%x -> DST: 0x%x", i, delta_src & 0xFFFFFFFE, delta_dst & 0xFFFFFFFE);
    i++;
    // first trace packet?
    if (delta_src & 1) {
      astrarv32_uart0_printf(" <TRACE_START>");
    }
    // branch due to exception?
    if (delta_dst & 1) {
      astrarv32_uart0_printf(" <TRAP_ENTRY>");
    }
...
int main(void) {
  ...
  astrarv32_rte_setup();
  astrarv32_rte_handler_install(TRAP_CODE_MENV_CALL, ecall_exc_handler); // install "ecall" handler
  astrarv32_rte_handler_install(TRACER_TRAP_CODE, tracer_irq_handler); // install tracer interrupt
  astrarv32_cpu_csr_set(CSR_MSTATUS, 1 << CSR_MSTATUS_MIE); // enable machine-mode interrupt
  ...
  astrarv32_uart0_printf("\n<< ASTRARV32 Tracer Demo >>\n\n");

  // check hardware/software configuration
  if (astrarv32_tracer_available() == 0) { // TRACER available?
    astrarv32_uart0_printf("[ERROR] TRACER module not available!\n");
    return -1;
  }


  // show trace buffer depth
  astrarv32_uart0_printf("Trace buffer: %d entries\n", astrarv32_tracer_get_buffer_depth());

  // configure TRACER
  uint32_t stop_address = (uint32_t)&ecall_exc_handler; // automatically stop tracing when reaching this address
  astrarv32_tracer_enable(0, stop_address); // 0 = trace CPU core 0
  ...
  astrarv32_uart0_printf("Starting trace...\n\n");
...
#if 1
  astrarv32_cpu_csr_set(CSR_MIE, 1 << TRACER_FIRQ_ENABLE);
#endif

  astrarv32_tracer_start(); // start trace logging
  test_code();            // this is code/function that we want to trace
  astrarv32_tracer_stop();  // stop trace logging
