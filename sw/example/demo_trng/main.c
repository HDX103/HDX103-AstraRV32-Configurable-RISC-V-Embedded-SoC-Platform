...
int main(void) {
  ...
  astrarv32_rte_setup();
  astrarv32_rte_handler_install(TRNG_TRAP_CODE, trng_firq_handler);
  ...
  if (astrarv32_trng_available() == 0) {
    astrarv32_uart0_printf("ERROR: no TRNG implemented!\n");
    return 1;
  }

  // show TRNG configuration
  astrarv32_uart0_printf("TRNG configuration\n");
  int i;
  for (i=0; i<astrarv32_trng_get_num_ros(); i++) {
    astrarv32_uart0_printf(" Ring-oscillator [%i]: %i inverters\n", i, astrarv32_trng_get_num_inv() + i*2);
  }
  astrarv32_uart0_printf(" Raw random bits per output byte: %i\n", astrarv32_trng_get_num_raw_bits());
  astrarv32_uart0_printf(" Random data FIFO depth: %i\n", astrarv32_trng_get_fifo_depth());

  // check if TRNG is using simulation mode
  if (astrarv32_trng_check_sim_mode() != 0) {
    astrarv32_uart0_printf(
      "\n"
  ...
  astrarv32_trng_fifo_clear(); // discard "warm-up" data
  ...
  while(1) {
    ...
    if (cmd == 'n') {
      ...
      astrarv32_uart0_printf("Invalid command.\n");
    }
  }
...
void irq_test(void) {
  ...
  // enable interrupt
  astrarv32_cpu_csr_set(CSR_MIE, 1 << TRNG_FIRQ_ENABLE);

  // wait for interrupt
  astrarv32_cpu_sleep();

  // disable interrupt
  astrarv32_cpu_csr_clr(CSR_MIE, 1 << TRNG_FIRQ_ENABLE);

  if (irq_ack == 1) {
    astrarv32_uart0_printf("IRQ test successful!\n");
  }
  else {
    astrarv32_uart0_printf("IRQ test FAILED!\n");
  }
}
...
void print_random_data(void) {
  ...
  while(1) {
    while(astrarv32_trng_data_avail() == 0);
    astrarv32_uart0_printf("%u ", (uint32_t)astrarv32_trng_data_get());
    num_samples++;
    if (astrarv32_uart0_char_received()) { // abort when key pressed
      astrarv32_uart0_char_received_get(); // discard received char
      break;
    }
  }
  astrarv32_uart0_printf("\nPrinted samples: %u\n", num_samples);
...
void print_hex(void) {
  ...
  while (1) {
    ...
    for (i=0; i<16; i++) {
      while (astrarv32_trng_data_avail() == 0);
      line[i] = astrarv32_trng_data_get();
    }
    ...
    if (astrarv32_uart0_char_received()) {
      astrarv32_uart0_char_received_get();
      return;
    }
  }
}
...
void repetition_count_test(void) {
  ...
  astrarv32_uart0_printf("Cut-off value = %u\n", c);

  astrarv32_trng_fifo_clear();

  while (astrarv32_trng_data_avail() == 0);
  a = astrarv32_trng_data_get();
  b = 1;
  while (1) {
    while (astrarv32_trng_data_avail() == 0);
    x = astrarv32_trng_data_get();
  ...
  if (fail) {
    astrarv32_uart0_printf("Test failed!\n");
  }
  else {
    astrarv32_uart0_printf("Test ok!\n");
  }
}
...
void adaptive_proportion_test(void) {
  ...
  astrarv32_uart0_printf("\nRunning test... Press any key to stop.\n");
  astrarv32_uart0_printf("Cut-off value = %u, windows size = %u\n", c, w);
...
void generate_histogram(void) {
  ...
  astrarv32_uart0_printf("Sampling... Press any key to stop.\n");
  ...
  while (1) {
    ...
    // get raw TRNG data
    while (astrarv32_trng_data_avail() == 0);
    trng_data = astrarv32_trng_data_get();
  ...
  // print histogram
  astrarv32_uart0_printf("Histogram [random data value] : [# occurrences]\n");
  for (i=0; i<256; i++) {
    astrarv32_uart0_printf("%u: %u\n", (uint32_t)i, hist[i]);
  }
  astrarv32_uart0_printf("\n");

  // print results
astrarv32_uart0_printf("[NOTE] integer numbers only\n");
  astrarv32_uart0_printf("Number of samples: %u\n", cnt);
  astrarv32_uart0_printf("Arithmetic mean:   %u (optimum would be 127)\n", (uint32_t)average);
  astrarv32_uart0_printf("\nHistogram occurrence\n");
  astrarv32_uart0_printf("Average:      %u (optimum would be %u/256 = %u)\n", occ_avg, n_samples, n_samples/256);
  astrarv32_uart0_printf("Min:          %u = average - %u (deviation) at bin %u (optimum deviation would be 0)\n", occ_min, occ_avg - occ_min, bin_min);
  astrarv32_uart0_printf("Max:          %u = average + %u (deviation) at bin %u (optimum deviation would be 0)\n", occ_max, occ_max - occ_avg, bin_max);
  astrarv32_uart0_printf("Average dev.: +/- %u (optimum would be 0)\n", occ_avg_dev);
}
...
void compute_rate(void) {
  ...
  uint32_t cycles = astrarv32_cpu_csr_read(CSR_CYCLE);

  i = 0;
  while (i<n_samples) {
    if (astrarv32_trng_data_avail()) { // data available?
      astrarv32_trng_data_get(); // discard data
      i++;
    }
  }

  uint32_t delta = astrarv32_cpu_csr_read(CSR_CYCLE) - cycles;
  uint32_t cycles_per_rnd = delta / n_samples;
  uint32_t rnd_per_sec = astrarv32_sysinfo_get_clk() / cycles_per_rnd;

  astrarv32_uart0_printf("Average random generation rate\n");
  astrarv32_uart0_printf("Cycles per random byte: ~%u\n", cycles_per_rnd);
  astrarv32_uart0_printf("Throughput (kB/s):      ~%u\n", rnd_per_sec/1024);
}
