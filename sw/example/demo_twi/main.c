...
int main() {
  ...
  if (astrarv32_twi_available() == 0) {
    astrarv32_uart0_printf("ERROR! TWI controller not available!");
    return 1;
  }

  // intro
  astrarv32_uart0_printf("\n--- TWI Bus Explorer ---\n\n");
  astrarv32_uart0_printf("This program allows to create TWI transfers by hand.\n"
                       "Execute 'help' to see the help menu.\n\n");

  // configure TWI, second slowest clock, no clock stretching allowed
  astrarv32_twi_setup(CLK_PRSC_2048, 15, 0);

  // Main menu
  for (;;) {
    astrarv32_uart0_printf("TWI_EXPLORER:> ");
    length = astrarv32_uart0_scan(buffer, 8, 1);
    astrarv32_uart0_printf("\n");
    ...
    if (!strcmp(buffer, "help")) {
      ...
      astrarv32_uart0_printf("Sending START condition...\n");
      astrarv32_twi_generate_start(); // generate START condition
    }
    else if (!strcmp(buffer, "stop")) {
      astrarv32_uart0_printf("Sending STOP condition...\n");
      astrarv32_twi_generate_stop(); // generate STOP condition
    }
      ...
      astrarv32_uart0_printf(" SCL: %u\n", astrarv32_twi_sense_scl() == 0 ? 0 : 1);
      astrarv32_uart0_printf(" SDA: %u\n", astrarv32_twi_sense_sda() == 0 ? 0 : 1);
    }
    else {
...
void set_clock(void) {
  ...
  // clock prescaler
  astrarv32_uart0_printf("Select new clock prescaler (0..7; one hex char): ");
  astrarv32_uart0_scan(terminal_buffer, 2, 1); // 1 hex char plus '\0'
  int prsc = (int)astrarv32_aux_hexstr2uint64(terminal_buffer, strlen(terminal_buffer));
  ...
  astrarv32_uart0_scan(terminal_buffer, 2, 1); // 1 hex char plus '\0'
  int cdiv = (int)astrarv32_aux_hexstr2uint64(terminal_buffer, strlen(terminal_buffer));
  ...
  astrarv32_twi_setup(prsc, cdiv, clkstr);

  // print new clock frequency
  uint32_t clock = astrarv32_sysinfo_get_clk() / (4 * PRSC_LUT[prsc] * (1 + cdiv));
  astrarv32_uart0_printf("\nNew I2C clock: %u Hz\n", clock);

  // check if bus lines are OK
  if (astrarv32_twi_sense_scl() == 0) {
    astrarv32_uart0_printf("WARNING! SCL bus line is not idle-high! Pull-up missing?\n");
  }
  if (astrarv32_twi_sense_sda() == 0) {
    astrarv32_uart0_printf("WARNING! SDA bus line is not idle-high! Pull-up missing?\n");
  }
}
...
void scan_twi(void) {
  ...
  astrarv32_uart0_printf("Scanning TWI bus...\n");

  for (i=0; i<128; i++) {
    astrarv32_twi_generate_start();
    uint8_t tmp = 2*i + 1;
    twi_ack = astrarv32_twi_transfer(&tmp, 0);
    astrarv32_twi_generate_stop();

    if (twi_ack == 0) {
      astrarv32_uart0_printf(" + Found device at write-address 0x");
      print_hex_byte(2*i);
      astrarv32_uart0_printf("\n");
      num_devices++;
    }
  ...
  if (num_devices == 0) {
    astrarv32_uart0_printf("No devices found.\n");
  }
  else {
    astrarv32_uart0_printf("Devices found: %i\n", num_devices);
  }
}
...
void send_twi(void) {
  ...
  // TX data
  astrarv32_uart0_printf("Enter TX data (2 hex chars): ");
  astrarv32_uart0_scan(terminal_buffer, 3, 1); // 2 hex chars for address plus '\0'
  data = (uint8_t)astrarv32_aux_hexstr2uint64(terminal_buffer, strlen(terminal_buffer));

  // host ACK
  astrarv32_uart0_printf("\nIssue ACK by host (y/n)? ");
  host_ack = 0;
  tmp = astrarv32_uart0_getc();
  astrarv32_uart0_putc(tmp);
  if (tmp == 'y') {
    host_ack = 1;
  }
  ...
  device_ack = astrarv32_twi_transfer(&data, host_ack);
  ...
  if (host_ack == 0) {
    astrarv32_uart0_printf("\n Response: ");
    if (device_ack == 0) {
      astrarv32_uart0_printf("ACK\n");
    }
    else {
      astrarv32_uart0_printf("NACK\n");
    }
  }
  else {
    astrarv32_uart0_printf("\n");
...
void print_hex_byte(uint8_t data) {
  ...
  astrarv32_uart0_putc(symbols[(data >> 4) & 15]);
  astrarv32_uart0_putc(symbols[(data >> 0) & 15]);
}
