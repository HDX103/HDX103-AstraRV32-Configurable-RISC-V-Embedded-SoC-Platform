...
int main(void) {
  ...
  astrarv32_uart0_printf("\n<< ASTRARV32 SMP Dual-Core Prime Numbers >>\n\n");
  ...
  int smp_launch_rc = astrarv32_smp_launch(core1_entry, (uint8_t*)core1_stack, sizeof(core1_stack));
  ...
  astrarv32_uart0_printf("Counting all prime numbers in range 0 to %u...\n", (uint32_t)NUM_MAX);
  ...
  time_delta = astrarv32_clint_time_get();

  // execute the full workload on core 0 only
  result = count_primes(0, NUM_MAX);

  time_delta = astrarv32_clint_time_get() - time_delta;

  astrarv32_uart0_printf("[SINGLE-CORE] %u primes in %u cycles\n", result, (uint32_t)time_delta);
  ...
  astrarv32_uart0_printf("[DUAL-CORE]   %u primes in %u cycles\n", result, (uint32_t)time_delta);
