...
int main(void) {
  ...
  while (1) {
    ...
    astrarv32_uart0_printf("This program requires a terminal resolution of at least %ux%u characters.\n", NUM_CELLS_X+2, NUM_CELLS_Y+3);
    astrarv32_uart0_printf("Press any key to start a random-initialized torus-style universe of %ux%u cells.\n", NUM_CELLS_X, NUM_CELLS_Y);
    astrarv32_uart0_printf("You can pause/restart the simulation by pressing any key.\n");
    ...
    while (astrarv32_uart0_char_received() == 0) {
      astrarv32_aux_xorshift32();
    }
    ...
    for (x=0; x<NUM_CELLS_X/8; x++) {
      for (y=0; y<NUM_CELLS_Y; y++) {
        if (trng_available) {
          ...
          universe[0][x][y] = astrarv32_trng_data_get(); // use data from TRNG
        }
        else {
          universe[0][x][y] = (uint8_t)astrarv32_aux_xorshift32(); // use data from PRNG
        }
      }
    }
...
void print_universe(int u){
  ...
  for (y=0; y<NUM_CELLS_Y; y++) {
    ...
    for (x=0; x<NUM_CELLS_X; x++) {
      if (get_cell(u, x, y))
        astrarv32_uart0_putc((char)CELL_ALIVE);
      else
        astrarv32_uart0_putc((char)CELL_DEAD);
    }
