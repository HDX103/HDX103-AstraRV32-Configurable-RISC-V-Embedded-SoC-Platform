...
int main(void) {
  ...
  if (astrarv32_clint_available()) {
    ...
    uint64_t timeout_time = astrarv32_clint_time_get() + (AUTO_BOOT_TIMEOUT * (uint64_t)(ASTRARV32_SYSINFO->CLK));
    while (1) {
      ...
      if (astrarv32_clint_time_get() >= timeout_time) {
        break;
      }
    }
  }
...
skip_auto_boot:
  ...
  while (1) {
    ...
    if (cmd == 'r') {
      asm volatile ("li t0, %[input_i]; jr t0" : : [input_i] "i" (ASTRARV32_BOOTROM_BASE));
    ...
    if (cmd == 'i') {
      uart_puts("HWV:  ");
      uart_puth(astrarv32_cpu_csr_read(CSR_MIMPID));
      uart_puts("\nCLK:  ");
      uart_puth(ASTRARV32_SYSINFO->CLK);
      uart_puts("\nMISA: ");
      uart_puth(astrarv32_cpu_csr_read(CSR_MISA));
      uart_puts("\nXISA: ");
      uart_puth(astrarv32_cpu_csr_read(CSR_MXISAH));
      uart_putc('_');
      uart_puth(astrarv32_cpu_csr_read(CSR_MXISA));
      uart_puts("\nSOC:  ");
      uart_puth(ASTRARV32_SYSINFO->SOC);
      uart_puts("\nMISC: ");
      uart_puth(ASTRARV32_SYSINFO->MISC);
      uart_puts("\n");
    }
