...
static int twi_transfer_byte(int write, uint8_t* data) {
  ...
  if (astrarv32_twi_transfer(&tmp, 0)) { // abort if NACK
    return 1;
  }
  ...
  rc |= astrarv32_twi_transfer(&addr.uint8[0], 0);
#elif (TWI_FLASH_ADDR_BYTES == 2)
  rc |= astrarv32_twi_transfer(&addr.uint8[1], 0);
  rc |= astrarv32_twi_transfer(&addr.uint8[0], 0);
#elif (TWI_FLASH_ADDR_BYTES == 3)
  rc |= astrarv32_twi_transfer(&addr.uint8[2], 0);
  rc |= astrarv32_twi_transfer(&addr.uint8[1], 0);
  rc |= astrarv32_twi_transfer(&addr.uint8[0], 0);
#elif (TWI_FLASH_ADDR_BYTES == 4)
  rc |= astrarv32_twi_transfer(&addr.uint8[3], 0);
  rc |= astrarv32_twi_transfer(&addr.uint8[2], 0);
  rc |= astrarv32_twi_transfer(&addr.uint8[1], 0);
  rc |= astrarv32_twi_transfer(&addr.uint8[0], 0);
#else
  ...
  // transfer data byte
  if (write) {
    if (astrarv32_twi_transfer(data, 0)) { // abort if NACK
      return 1;
    }
  }
  else {
    tmp = 0xff;
    astrarv32_twi_transfer(&tmp, 0);
    *data = tmp;
  }
  ...
  if (write) {
    while(1) {
      astrarv32_twi_generate_start();
      tmp = (uint8_t)TWI_FLASH_ID | 0;
      rc = astrarv32_twi_transfer(&tmp, 0);
      astrarv32_twi_generate_stop();
      if (rc == 0) {
        break;
      }
...
int twi_flash_setup(void) {
  ...
  astrarv32_twi_setup(TWI_FLASH_CLK_PRSC, TWI_FLASH_CLK_DIV, 0);

  // try to access flash
  astrarv32_twi_generate_start();
  uint8_t tmp = (uint8_t)TWI_FLASH_ID | 0;
  int rc = astrarv32_twi_transfer(&tmp, 0);
  astrarv32_twi_generate_stop();
  if (rc) {
