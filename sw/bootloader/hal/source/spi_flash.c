...
static void spi_flash_cmd(uint8_t cmd) {
  ...
  astrarv32_spi_cs_en(SPI_FLASH_CS);
  astrarv32_spi_transfer(cmd);
...
static uint8_t spi_flash_read_status(void) {
  ...
  astrarv32_spi_transfer(SPI_FLASH_CMD_READ_STATUS);
  uint8_t res = astrarv32_spi_transfer(0);
...
static void spi_flash_send_addr(void) {
  ...
  astrarv32_spi_transfer(addr.uint8[0]);
#elif (SPI_FLASH_ADDR_BYTES == 2)
  astrarv32_spi_transfer(addr.uint8[1]);
  astrarv32_spi_transfer(addr.uint8[0]);
#elif (SPI_FLASH_ADDR_BYTES == 3)
  astrarv32_spi_transfer(addr.uint8[2]);
  astrarv32_spi_transfer(addr.uint8[1]);
  astrarv32_spi_transfer(addr.uint8[0]);
#elif (SPI_FLASH_ADDR_BYTES == 4)
  astrarv32_spi_transfer(addr.uint8[3]);
  astrarv32_spi_transfer(addr.uint8[2]);
  astrarv32_spi_transfer(addr.uint8[1]);
  astrarv32_spi_transfer(addr.uint8[0]);
#else
...
int spi_flash_setup(void) {
  ...
  astrarv32_spi_setup(SPI_FLASH_CLK_PRSC, SPI_FLASH_CLK_DIV, 0, 0);
...
int spi_flash_erase(void) {
  ...
  while (num_sectors--) {
    ...
    astrarv32_spi_transfer(SPI_FLASH_CMD_SECTOR_ERASE);
    spi_flash_send_addr();
...
int spi_flash_stream_get(uint32_t* rdata) {
  ...
  astrarv32_spi_transfer(SPI_FLASH_CMD_READ);
  spi_flash_send_addr();

  int i;
  subwords32_t tmp;
  for (i=0; i<4; i++) {
    tmp.uint8[i] = astrarv32_spi_transfer(0);
  }
...
int spi_flash_stream_put(uint32_t wdata) {
  ...
  for (i=0; i<4; i++) {
    ...
    astrarv32_spi_transfer(SPI_FLASH_CMD_PAGE_PROGRAM);
    spi_flash_send_addr();
    astrarv32_spi_transfer(tmp.uint8[i]);
