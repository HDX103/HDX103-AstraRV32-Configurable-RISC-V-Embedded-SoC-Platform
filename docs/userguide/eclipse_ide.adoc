...
to develop, debug and profile application code for the ASTRARV32 RISC-V Processor. This chapter shows how to import
the provided **example setup** from the ASTRARV32 project repository. Additionally, all the required steps to create
...
.Developing and debugging code for the ASTRARV32 using the Eclipse IDE
...
[TIP]
Some ASTRARV32 makefile targets relies on the `basename` command which might not be part of the default
...
A preconfigured Eclipse project is available in `astrarv32/sw/example/eclipse`.
To import it:
...
.ASTRARV32 Folder and File Paths
[IMPORTANT]
The provided example project uses **relative paths** for including all the ASTRARV32-specific files and folders
...
https://stnolting.github.io/astrarv32/#_application_makefile


:sectnums:
=== Setup a new Eclipse Project from Scratch

This chapter shows all the steps required to create an Eclipse project for the ASTRARV32 entirely from scratch.
...
. Copy the makefile of an existing ASTRARV32 example program and paste it to the new (empty) makefile.

==== Add Build Targets (optional)

This step adds some of the targets of the ASTRARV32 makefile for easy access. This step is optional.
...
.Available Target
[TIP]
See the ASTRARV32 data sheet for a list and description of all available makefile targets:
https://stnolting.github.io/astrarv32/#_makefile_targets
...
==== Add ASTRARV32 Software Framework

[start=1]
. In the project explorer right-click on the project and select **Properties**.
. Expand **C/C++ General**, click on **Paths and Symbols** and highlight **Assembly** under **Languages**.
. In the **Include** tab click **Add...**
* Check the box in front of **Add to all languages** and click on **File System...** and select the ASTRARV32 library include folder (`path/to/astrarv32/sw/lib/include`).
* Click **OK**.
. In the **Include** tab click **Add...**.
* Check the box in front of **Add to all languages** and click on **File System...** and select the ASTRARV32 commons folder (`path/to/astrarv32/sw/common`).
* Click **OK**.
...
* Select the source folder of the ASTRARV32 software framework (`path/to/astrarv32/sw/lib/source`).
* Click **OK**.
. Click **Apply and Close**.
...
* In the **Debugger** tab add the ASTRARV32 OpenOCD script with a `-f` in front of it-
* Example: `Config options: -f ${ProjDirPath}/../../openocd/astrarv32.cfg`
...
* In the **SVD Path** tab add the ASTRARV32 SVD file (`path/to/astrarv32/sw/svd/astrarv32.svd`).
