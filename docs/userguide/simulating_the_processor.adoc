...
Due to the complexity of the ASTRARV32 processor and all the different configuration options,
...
ensure that all application images (`*_image.vhd`) are reanalyzed when recompiling the ASTRARV32 application or bootloader.

:sectnums:
=== Testbench

A plain-VHDL testbench without any third-party libraries / dependencies (`sim/astrarv32_tb.vhd`) can be used for simulating
...
[IMPORTANT]
Data written to the ASTRARV32 UART0 / UART1 transmitter is send to a virtual UART receiver implemented as part of the default
...
section https://stnolting.github.io/astrarv32/#_primary_universal_asynchronous_receiver_and_transmitter_uart0).
TX data sent to UART0 / UART1 will be immediately printed to the simulator console instead of being transmitted via the
physical UART TX line.
...
[source, bash]
----
astrarv32/sim$ sh ghdl.sh --stop-time=20ms
----
...
To do a quick test of the ASTRARV32 make and the required tools navigate to the project's `sw/example/hello_world`
folder and run `make USER_FLAGS+=-DUART0_SIM_MODE clean install sim`:

[source, bash]
----
astrarv32/sw/example/hello_world$ make USER_FLAGS+=-DUART0_SIM_MODE clean install sim
../../../sw/lib/source/astrarv32_uart.c: In function 'astrarv32_uart_setup':
../../../sw/lib/source/astrarv32_uart.c:80:2: warning: #warning UART0_SIM_MODE (primary UART) enabled! Sending all UART0.TX data to text.io simulation output instead of real UART0 transmitter. Use this for simulation only! [-Wcpp] <1>
...
Generating astrarv32_imem_image.vhd
Installing application image to ../../../rtl/core/astrarv32_imem_image.vhd <3>
Simulating processor using default testbench...
GHDL simulation run parameters: --stop-time=10ms <4>
../rtl/core/astrarv32_top.vhd:329:5:@0ms:(assertion note): [ASTRARV32] The ASTRARV32 RISC-V Processor (v1.11.6.0), github.com/stnolting/astrarv32
../rtl/core/astrarv32_top.vhd:335:5:@0ms:(assertion note): [ASTRARV32] Processor Configuration: CPU (smp-dual-core) IMEM-ROM DMEM I-CACHE D-CACHE XBUS CLINT GPIO UART0 UART1 SPI SDI TWI TWD PWM WDT TRNG CFS NEOLED GPTMR ONEWIRE DMA SLINK SYSINFO OCD OCD-AUTH OCD-HWBP
../rtl/core/astrarv32_top.vhd:388:5:@0ms:(assertion note): [ASTRARV32] BOOT_MODE_SELECT = 2: booting IMEM image
../rtl/core/astrarv32_cpu.vhd:130:5:@0ms:(assertion note): [ASTRARV32] CPU ISA: rv32iabmux_zaamo_zalrsc_zba_zbb_zbkb_zbkc_zbkx_zbs_zicntr_zicond_zicsr_zifencei_zihpm_zfinx_zkn_zknd_zkne_zknh_zks_zksed_zksh_zkt_zmmul_sdext_sdtrig_smpmp_xcfu
../rtl/core/astrarv32_cpu.vhd:168:5:@0ms:(assertion note): [ASTRARV32] CPU tuning options: fast_mul fast_shift
../rtl/core/astrarv32_cpu.vhd:175:5:@0ms:(assertion warning): [ASTRARV32] Assuming this is a simulation.
../rtl/core/astrarv32_imem.vhd:61:3:@0ms:(assertion note): [ASTRARV32] Implementing processor-internal IMEM as pre-initialized ROM.
../rtl/core/astrarv32_trng.vhd:298:3:@0ms:(assertion note): [neoTRNG] The neoTRNG (v3.3) - A Tiny and Platform-Independent True Random Number Generator, https://github.com/stnolting/neoTRNG
../rtl/core/astrarv32_trng.vhd:308:3:@0ms:(assertion warning): [neoTRNG] Simulation-mode enabled (NO TRUE/PHYSICAL RANDOM)!
../rtl/core/astrarv32_debug_auth.vhd:44:3:@0ms:(assertion warning): [ASTRARV32] using DEFAULT on-chip debugger authenticator. Replace by custom module.
<5>
                                                                                      ##        ##   ##   ##
 ##     ##   #########   ########    ########   ##      ##   ########    ########     ##      ################
####    ##  ##          ##      ##  ##      ##  ##      ##  ##      ##  ##      ##    ##    ####            ####
