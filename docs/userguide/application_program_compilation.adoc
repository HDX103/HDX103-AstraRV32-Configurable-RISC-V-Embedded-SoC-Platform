...
This guide shows how to compile a C-code application for the ASTRARV32 processor.
...
[source,bash]
----
astrarv32/sw/example/demo_blink_led$ make clean_all exe
----
...
At the end an ELF file (`main.elf`) is generated. The _ASTRARV32 image generator_ (in `sw/image_gen`)
takes this file and creates the final executable (`astrarv32_exe.bin`). The makefile will show the resulting
...
Executable (astrarv32_exe.bin) size in bytes:
1016
----
...
[start=5]
. The `exe` target has created the actual executable `astrarv32_exe.bin` in the current folder
