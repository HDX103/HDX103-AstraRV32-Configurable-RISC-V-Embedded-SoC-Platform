...
into a block-design-based Vivado project. The ASTRARV32 repository provides a full-scale TCL script that
...
(`rtl/system_integration/astrarv32_vivado_ip.vhd`) that features AXI4-compatible (via XBUS)
...
.Example Vivado SoC using the ASTRARV32 IP Block
...
. Use `cd` in the TCL console to navigate to the project's `astrarv32/rtl/system_integration` folder.
. Execute `source astrarv32_vivado_ip.tcl` in the TCL console.
...
. A new folder `astrarv32_vivado_ip_work` is created in `astrarv32/rtl/system_integration` which contains the IP-packaging Vivado project.
. The `packaged_ip` sub-folder provides the actual IP module which is automatically added to the project's IP repository.
. You will find the ASTRARV32 in the "User Repository" section of the Vivado IP catalog.
...
. Navigate to the `astrarv32/rtl/system_integration` folder and select `astrarv32_vivado_ip.tcl`.
. A second Vivado instance will open automatically packaging the IP module. After this process is completed, the second Vivado instance will automatically close again.
. A new folder `astrarv32_vivado_ip_work` is created in `astrarv32/rtl/system_integration` which contains the IP-packaging Vivado project.
. The `packaged_ip` sub-folder provides the actual IP module.
. Open your custom design where you want to integrate the ASTRARV32 IP module.
...
. Click the large plus button and select the previously generated IP folder (`path/to/astrarv32/rtl/system_integration/astrarv32_vivado_ip_work/packaged_ip`).
...
the ASTRARV32 IP module needs to be re-packaged by re-executing the packing script (`astrarv32_vivado_ip.tcl`). +
 +
...
(`astrarv32_imem_image.vhd`) file in the packaged_ip folder. For the Vivado design suite, the new program to be executed must
be compiled and installed using the `install` makefile target. Next, the `astrarv32_vivado_ip.tcl` script has to be executed again.
Finally, Vivado will prompt to upgrade the ASTRARV32 IP.

.Out-of-Context Synthesis
[IMPORTANT]
If ASTRARV32 configurations do not take effect, try using the out-of-context synthesis option.

.Incremental Compilation of Simulation Sources (ISIM)
[IMPORTANT]
When using AMD Vivado (XSIM for simulation) make sure to **TURN OFF** "incremental compilation" (_Project Setting_
