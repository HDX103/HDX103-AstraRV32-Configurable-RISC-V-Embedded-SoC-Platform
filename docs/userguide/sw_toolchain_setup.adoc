...
To compile (and debug) executables for the ASTRARV32 a RISC-V-compatible toolchain is required.
...
This default prefix can be chanced by the `RISCV_PREFIX` variable in the ASTRARV32 application makefile(s).
...
Hence, you need to adjust the `RISCV_PREFIX` variable accordingly before running any ASTRARV32 makefiles.
...
To integrate the toolchain of choice into the ASTRARV32 software framework, the toolchain's binaries need
...
To make sure everything works fine, navigate to an example project in the ASTRARV32 `sw/example` folder and
execute the following command:

[source,bash]
----
astrarv32/sw/example/demo_blink_led$ make check
----

This will test all the tools required for generating ASTRARV32 executables.
