...
and DDR memory controller. The ASTRARV32 has been ported to the LiteX framework to be used as central processing unit.

The default microcontroller-like ASTRARV32 processor is not directly supported as all the peripherals would provide some _redundancy_.
Instead, the LiteX port uses a _core complex wrapper_ that only includes the actual ASTRARV32 CPU, the instruction cache (optional),
...
[NOTE]
The ASTRARV32 core complex wrapper used by LiteX for integration can be found in
`rtl/system_integration/astrarv32_litex_core_complex.vhd`.

.LiteX Port Documentation
[TIP]
More information can be found in the "ASTRARV32" section of the LiteX project wiki: https://github.com/enjoy-digital/litex/wiki/CPUs
...
. The ASTRARV32 port for LiteX uses GHDL and yosys for converting the VHDL files via the https://github.com/ghdl/ghdl-yosys-plugin[GHDL-yosys-plugin].
...
module (not tested on LiteX yet). Check out https://github.com/stnolting/astrarv32-verilog[astrarv32-verilog] for more information.
...
- from https://github.com/YosysHQ/fpga-toolchain +
 +
**This means you might have to edit the call to yosys in `litex/soc/cores/cpu/astrarv32/core.py`.**
...
Start a simulation right in your console using the ASTRARV32 as target CPU:

[source, bash]
----
$ litex_sim --cpu-type=astrarv32
----
