...
The ASTRARV32 on-chip debugger ("OCD") allows online in-system debugging via an external JTAG access port.
...
Connect a JTAG adapter to the according ASTRARV32 `jtag_*` top interface signals:

.JTAG pin mapping
[cols="^3,^2,^2"]
[options="header",grid="rows"]
|=======================
| ASTRARV32 top signal | JTAG signal | Default FTDI port
| `jtag_tck_i`       | TCK         | D0
| `jtag_tdi_i`       | TDI         | D1
| `jtag_tdo_o`       | TDO         | D2
| `jtag_tms_i`       | TMS         | D3
...
.JTAG TAP Reset
[NOTE]
The ASTRARV32 JTAG TAP does not provide a dedicated reset signal ("TRST"). However, this
...
The ASTRARV32 on-chip debugger can be accessed using the upstream version of OpenOCD.
A pre-configured OpenOCD configuration file is provided: `sw/openocd/astrarv32.*.cfg`

.Interface Configuration
[NOTE]
You might need to adapt the default interface configuration in `sw/openocd/lib/interface.cfg`
...
.Connecting via OpenOCD (on Windows) using the default `astrarv32.cfg` script
[source, bash]
--------------------------
astrarv32\sw\openocd>openocd.exe -f astrarv32.cfg
xPack Open On-Chip Debugger 0.12.0+dev-02228-ge5888bda3-dirty (2025-10-04-22:44)
Licensed under GNU GPL v2
For bug reports, read
        http://openocd.org/doc/doxygen/bugs.html
Target: astrarv32 single-core
Error: libusb_open() failed with LIBUSB_ERROR_NOT_FOUND
Info : clock speed 2000 kHz
Info : JTAG tap: astrarv32.cpu tap/device found: 0x00000001 (mfg: 0x000 (<invalid>), part: 0x0000, ver: 0x0)
Info : datacount=1 progbufsize=2
Info : Disabling abstract command reads from CSRs.
Info : Examined RISC-V core; found 2 harts
Info :  hart 0: XLEN=32, misa=0x40901106
Info : [astrarv32.cpu] Examination succeed
Info : [astrarv32.cpu] starting gdb server on 3333
...
debugging of peripheral/IO devices (see https://github.com/stnolting/astrarv32/discussions/656).
...
[source, bash]
--------------------------
astrarv32/sw/example/demo_blink_led$ make clean_all all
--------------------------
...
[source, bash]
--------------------------
.../astrarv32/sw/example/demo_blink_led$ riscv32-unknown-elf-gdb
GNU gdb (GDB) 10.1
...
Software for the ASTRARV32 processor can also be developed and debugged _in-system_ using Segger Embedded Studio
...
* Segger notes regarding ASTRARV32: https://wiki.segger.com/J-Link_ASTRARV32
