...
[grid="none"]
|=======================
| Hardware source files:  | astrarv32_tracer.vhd    |
| Software driver files:  | astrarv32_tracer.c      | link:https://stnolting.github.io/astrarv32/sw/astrarv32__tracer_8c.html[Online software reference (Doxygen)]
|                         | astrarv32_tracer.h      | link:https://stnolting.github.io/astrarv32/sw/astrarv32__tracer_8h.html[Online software reference (Doxygen)]
...
The ASTRARV32 tracer module allows to record program execution in order to find out how the CPU
...
[source, gdb]
----
(gdb) source path/to/astrarv32/sw/example/demo_tracer/astrarv32_tracer.gdb
----
...
.Using the Tracer from GDB (GDB started in `astrarv32/sw/example/demo_tracer`)
[source, gdb]
----
...
(gdb) c <3>
...
(gdb) source astrarv32_tracer.gdb <4>
(gdb) tracer_get <5>
...
The ASTRARV32 tracer module can also be used to generate a complete log of all executed instructions. Simulation
...
* `astrarv32.tracer0.log` for CPU 0
* `astrarv32.tracer1.log` for CPU 1 (only if <<_dual_core_configuration>> is enabled)
...
.Exemplary cut-out from a simulation trace log (here: `astrarv32.tracer1.log` showing boot of core 1)
[source, log]
----
...
.TRACER register map (`struct ASTRARV32_TRACER`)
[cols="<4,<2,<6,^2,<8"]
