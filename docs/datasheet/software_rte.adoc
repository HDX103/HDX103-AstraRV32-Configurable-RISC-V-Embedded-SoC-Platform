<<<
:sectnums:
=== ASTRARV32 Runtime Environment

The ASTRARV32 software framework provides a minimal runtime environment ("RTE") that takes care of a stable
...
stored to this register defines the address of the **first-level trap handler**, which is provided by the ASTRARV32
...
The ASTRARV32 runtime environment is part of the default ASTRARV32 software framework. The links to the according
software references are listed below.

[cols="<1,<8"]
[grid="none"]
|=======================
| astrarv32_rte.c | link:https://stnolting.github.io/astrarv32/sw/astrarv32__rte_8c.html[Online software reference (Doxygen)]
| astrarv32_rte.h | link:https://stnolting.github.io/astrarv32/sw/astrarv32__rte_8h.html[Online software reference (Doxygen)]
|=======================
...
int main() {
  ...
  astrarv32_rte_setup(); // setup ASTRARV32 runtime environment
...
[source,c]
----
int astrarv32_rte_handler_install(uint8_t id, void (*handler)(void));
----

The first argument `id` defines the "trap ID" (for example a certain interrupt request) that shall be handled by the
user-defined handler. These IDs are defined in `sw/lib/include/astrarv32_rte.h`. However, more convenient device-specific
aliases are also defined in `sw/lib/include/astrarv32.h`. The second argument `handler` is the actual function that
...
[source,c]
----
astrarv32_rte_handler_install(TRAP_CODE_MTI, machine_timer_irq_handler); // handler for machine timer interrupt
astrarv32_rte_handler_install(TRAP_CODE_MENV_CALL, environment_call_handler); // handler for machine environment call exception
astrarv32_rte_handler_install(SLINK_TRAP_CODE, slink_rx_handler); // handler for SLINK receive interrupt
----
...
[source]
----
<ASTRARV32-RTE-PANIC> [cpu0|M] Illegal instruction MEPC=0x000002d6 MTVAL=0x000000FF </ASTRARV32-RTE-PANIC> <1>
<ASTRARV32-RTE-PANIC> [cpu0|U] Illegal instruction MEPC=0x00000302 MTVAL=0x00000000 </ASTRARV32-RTE-PANIC> <2>
<ASTRARV32-RTE-PANIC> [cpu0|U] Load address misaligned MEPC=0x00000440 MTVAL=0x80000101 </ASTRARV32-RTE-PANIC> <3>
<ASTRARV32-RTE-PANIC> [cpu1|M] FIRQ channel 0x00000003 MEPC=0x00000820 MTVAL=0x00000000 </ASTRARV32-RTE-PANIC> <4>
<ASTRARV32-RTE-PANIC> [cpu1|M] Instruction access fault MEPC=0x90000000 MTVAL=0x00000000 FATAL! HALTING CPU </ASTRARV32-RTE-PANIC>\n <5>
----
<1> Illegal 32-bit instruction `MTVAL=0x000000FF` at address `MEPC=0x000002d6` while CPU 0 was in machine-mode (`[cpu0|M]`).
...
----
// Prototypes
uint32_t astrarv32_rte_context_get(int x); // read register
void     astrarv32_rte_context_put(int x, uint32_t data); // write data to register

// Examples
uint32_t tmp = astrarv32_rte_context_get(9); // read register 'x9'
astrarv32_rte_context_put(28, tmp); // write 'tmp' to register 'x28'
----
...
[TIP]
A demo program, which showcases how to emulate unaligned memory accesses using the ASTRARV32 runtime environment
