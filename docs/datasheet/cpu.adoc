<<<
:sectnums:
== ASTRARV32 Central Processing Unit (CPU)

The ASTRARV32 CPU is an area-optimized RISC-V core implementing the `rv32i_zicsr_zifencei` base (privileged) ISA and
...
The following table shows all interface signals of the CPU top entity `rtl/core/astrarv32_cpu.vhd`. All ports
...
.ASTRARV32 CPU Signal List (Dir = direction seen from the CPU)
[cols="^2,^2,^1,<8"]
...
| `ifence_o`   | 1              | out | Instruction fence / memory ordering.
...
| `dfence_o`   | 1              | out | Data fence / memory ordering.
...
ASTRARV32 processor setup. These generics are assigned by the processor setup only and are not available for
...
.ASTRARV32 CPU-Exclusive Generic List
[cols="<4,^2,<8"]
[options="header",grid="rows"]
|=======================
| Name | Type | Description
| `HART_ID`          | natural   | ID of the core (for <<_mhartid>> CSR).
| `VENDOR_ID`        | suv(31:0) | Vendor ID (for <<_mvendorid>> CSR).
...
image::astrarv32_cpu.png[align=center]
...
The front-end is responsible for fetching instructions in chunks of 32-bits. This can be a single aligned 32-bit
...
to the CPU back-end for execution.
...
The actual instruction execution is conducted by a micro-sequencer-based approach. Execution is split into several
...
an optimal CPI (cycles per instruction) value of 2 can be achieved.

The division into individual micro-operations not only allows for small hardware that reuses CPU parts in different
...
The data register file (VHDL file `astrarv32_cpu_regfile.vhd`) implements the general purpose RISC-V architecture registers
...
platform/technology. The `CPU_RF_ARCH_SEL` <<_cpu_tuning_options, CPU tuning option>> generic is used to select a
specific architecture style:

.CPU Register File Implementation Flavors
[cols="<2,<4,<4"]
[options="header",grid="rows"]
|=======================
| `CPU_RF_ARCH_SEL`    | Implementation Style                       | Note
| **0** (`sram_sync`)  | <<_register_based_with_synchronous_read>>  | can infer FPGA block-RAM
| **1** (`sram_async`) | <<_register_based_with_asynchronous_read>> | can infer FPGA distributed-RAM
| **2** (`reg`)        | <<_register_based_with_reset>>             | provides a full hardware reset
| **3** (`latch`)      | <<_latch_based>>                           | improved efficiency for ASIC implementations
|=======================

.No Hardware Reset
[IMPORTANT]
Apart from the <<_register_based_with_reset>> style, the register file does not provide a dedicated hardware reset.
Hence, all `x` registers power-up in an undefined state and need software-based initialization (e.g. by the <<_start_up_code_crt0>>).
...
operands (`rs1` & `rs2`) is performed by three individual port, whereby the read accesses are implemented _asynchronously_.
...
The arithmetic/logic unit ("ALU", VHDL file `astrarv32_cpu_alu.vhd`) is used for actual data processing as well as generating
memory and branch addresses. All "simple" <<_i_isa_extension>> computational instructions (like `add` and `or`) are implemented
...
or multiplications are processed by so-called "ALU co-processors" (VHDL files `astrarv32_cpu_alu_*.vhd`).
...
<<_custom_functions_unit_cfu>>) does not complete processing in a bound amount of time (configured via
...
The load/store unit (VHDL file `astrarv32_cpu_lsu.vhd`) takes care of handling data memory accesses via load and store
...
See the ASTRARV32 Runtime Environment's <<_application_context_handling>> section for more information.
...
Trace data is generated by the CPU's trace generator (VHDL file `astrarv32_cpu_trace.vhd`), which is enabled if either the
<<_execution_trace_buffer_tracer>> is implemented of if the external trace ports are enabled by the `TRACE_PORT_EN`
<<_processor_top_entity_generics, top generic>> (via the CPU core's <<_cpu_trace_en>> tuning option).
...
package file (`astrarv32_package.vhd`).
...
| `order`     |    64 | `rvfi_order`     | Instruction index (linear incrementing counter; resets to zero on overflow); bits `63:32` are hardwired to all-zero
...
help to shorten the CPU's critical path simplifying timing closure.
...
[frame="topbot",grid="none"]
|=======================
| Name        | Register file implementation style
| Type        | `natural` (0..2)
| Default     | `0` (simple-dual-port RAM with synchronous read)
| Description | This variable can be used to select a specific architecture for the CPU's general-purpose register file that
can be implemented most efficiently for the target platform/technology. see section <<_cpu_register_file>>.
|=======================
...
The ASTRARV32 CPU provides a single sleep mode that can be entered to power-down the core reducing
...
Just like the RISC-V ISA, the ASTRARV32 aims to provide _maximum virtualization_ capabilities on CPU and SoC level to
allow a high level of **execution safety**. The CPU supports **all** traps specified by the official RISC-V
...
All reserved, unsupported, unavailable, or illegal instructions always raise a precise exception, which, for example,
also allows for software-based emulation of additional ISA extensions or system features.
...
The ASTRARV32 CPU provides separated instruction fetch and data access interfaces making it a **Harvard Architecture**:
...
[wavedrom, format="png", align="center"]
----
{signal: [
  {name: 'clk', wave: 'p..|.......|..'},
  [
...
{signal: [
  ...
  [
    "response",
    {name: 'ack',  wave: '0.1010.|..101..0.', node: '..c.eg....j.l..o'},
    {name: 'err',  wave: 'x.0x0x.|x.0x0..x.'},
    {name: 'data', wave: 'x.2x...|x.3x456x.', data: ['rdata', '[0]', '[4]', '[8]', '[12]']},
  ]
],
 edge: ['ab', 'a~>c', 'c~>d', 'd~>e', 'fg', 'hi', 'h~>j', 'j~>k', 'k~>l', 'm', 'no']
}
----
...
defines the base/start address of the burst.`lock` stays high until the very last `stb` has been acknowledged ("n").
...
. For the very last transfer element `burst` is deasserted ("m"). This indicates the end of the burst.
. The burst transfer is completed when all required `stb` pulses are sent and when all corresponding `ack` have been received ("o").
...
<<_astrarv32_trap_listing>>. The address that reflects the current program counter when a trap was taken is stored to
<<_mepc>> CSR. Additional information regarding the cause of the trap can be retrieved from the <<_mtval>> CSR.
...
As a custom extension, the ASTRARV32 CPU features 16 fast interrupt request (FIRQ) lines via the `firq_i` CPU top
entity signals. These interrupts have custom configuration and status flags in the <<_mie>> and <<_mip>> CSRs and also
provide custom trap codes in <<_mcause>>. These FIRQs are reserved for ASTRARV32 processor-internal usage only.


:sectnums:
==== ASTRARV32 Trap Listing

The following tables show all traps that are currently supported by the ASTRARV32 CPU. It also shows the prioritization
and the CSR side-effects.

.FIRQ Mapping
[TIP]
See section <<_astrarv32_specific_fast_interrupt_requests>> for the mapping of the FIRQ channels to the according hardware modules.

**Table Annotations**

The "RTE Trap ID" aliases are defined by the ASTRARV32 core library and can be used in plain C code
when interacting with the pre-defined RTE functions. The <<_mcause>>, <<_mepc>> and <<_mtval>>
...
|          | `0x0000000b` | `TRAP_CODE_MENV_CALL`    | environment call from M-mode      | PC     | 0
|          | `0x00000008` | `TRAP_CODE_UENV_CALL`    | environment call from U-mode      | PC     | 0
|          | `0x00000003` | `TRAP_CODE_BREAKPOINT`   | software breakpoint / HW trigger  | PC     | 0
|          | `0x00000006` | `TRAP_CODE_S_MISALIGNED` | store address misaligned          | PC     | ADDR
...
|          | `0x80000013` | `TRAP_CODE_FIRQ_3`       | fast interrupt request channel 3  | I-PC   | 0
|          | `0x80000014` | `TRAP_CODE_FIRQ_4`       | fast interrupt request channel 4  | I-PC   | 0
|          | `0x80000015` | `TRAP_CODE_FIRQ_5`       | fast interrupt request channel 5  | I-PC   | 0
|          | `0x80000016` | `TRAP_CODE_FIRQ_6`       | fast interrupt request channel 6  | I-PC   | 0
|          | `0x80000017` | `TRAP_CODE_FIRQ_7`       | fast interrupt request channel 7  | I-PC   | 0
|          | `0x80000018` | `TRAP_CODE_FIRQ_8`       | fast interrupt request channel 8  | I-PC   | 0
|          | `0x80000019` | `TRAP_CODE_FIRQ_9`       | fast interrupt request channel 9  | I-PC   | 0
|          | `0x8000001a` | `TRAP_CODE_FIRQ_10`      | fast interrupt request channel 10 | I-PC   | 0
|          | `0x8000001b` | `TRAP_CODE_FIRQ_11`      | fast interrupt request channel 11 | I-PC   | 0
|          | `0x8000001c` | `TRAP_CODE_FIRQ_12`      | fast interrupt request channel 12 | I-PC   | 0
|          | `0x8000001d` | `TRAP_CODE_FIRQ_13`      | fast interrupt request channel 13 | I-PC   | 0
|          | `0x8000001e` | `TRAP_CODE_FIRQ_14`      | fast interrupt request channel 14 | I-PC   | 0
|          | `0x8000001f` | `TRAP_CODE_FIRQ_15`      | fast interrupt request channel 15 | I-PC   | 0
|          | `0x8000000b` | `TRAP_CODE_MEI`          | machine external interrupt (MEI)  | I-PC   | 0
|          | `0x80000003` | `TRAP_CODE_MSI`          | machine software interrupt (MSI)  | I-PC   | 0
| lowest   | `0x80000007` | `TRAP_CODE_MTI`          | machine timer interrupt (MTI)     | I-PC   | 0
|=======================

.ASTRARV32 Trap Description
[cols="<3,<7"]
[options="header",grid="rows"]
|=======================
...
| `TRAP_CODE_FIRQ_*`       | caused by interrupt-condition of **processor-internal modules**, see <<_astrarv32_specific_fast_interrupt_requests>>
