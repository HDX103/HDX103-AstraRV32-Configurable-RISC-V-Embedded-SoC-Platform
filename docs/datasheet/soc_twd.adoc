...
[grid="none"]
|=======================
| Hardware source files:  | astrarv32_twd.vhd    |
| Software driver files:  | astrarv32_twd.c      | link:https://stnolting.github.io/astrarv32/sw/astrarv32__twd_8c.html[Online software reference (Doxygen)]
|                         | astrarv32_twd.h      | link:https://stnolting.github.io/astrarv32/sw/astrarv32__twd_8h.html[Online software reference (Doxygen)]
...
The ASTRARV32 TWD implements an I2C-compatible **device-side** interface. Processor-external I2C hosts can communicate
with this module by issuing I2C transactions. The TWD is entirely passive an only reacts to external transmissions.

.Device-Mode Only
[NOTE]
The ASTRARV32 TWD controller only supports **device mode**. Transmission are initiated by processor-external modules
...
The TWD module requires a single tristate drives (actually: open-drain drivers - signals can only be actively driven low) for
the SDA line, which have to be implemented by the user in the setup's top module / IO ring. The SCL signal is only read by the
TWD module (input only) and therefore does not require a tristate driver. A generic VHDL example is shown below.
...
twd_scl_i <= std_ulogic(scl_io);
----


**Register Map**

.TWD register map (`struct ASTRARV32_TWD`)
[cols="<2,<1,<4,^1,<6"]
[options="header",grid="all"]
|=======================
