...
[grid="none"]
|=======================
| Hardware source files:  | astrarv32_pwm.vhd |
| Software driver files:  | astrarv32_pwm.c   | link:https://stnolting.github.io/astrarv32/sw/astrarv32__pwm_8c.html[Online software reference (Doxygen)]
|                         | astrarv32_pwm.h   | link:https://stnolting.github.io/astrarv32/sw/astrarv32__pwm_8h.html[Online software reference (Doxygen)]
...
The ASTRARV32 PWM module supports toggle-free 0% and 100% duty cycle output rates. 0% duty cycle is achieved by
setting `CMP = 0`. 100% duty cycle is achieved by setting `CMP = TOP + 1`.

.`CHANNEL` Half-Word Access
[TIP]
In contrast to many other ASTRARV32 peripherals, the PWM `CHANNEL[i]` registers can also be accessed using half-word
...
.PWM register map (`struct ASTRARV32_PWM`)
[cols="<3,<3,^2,^2,<7"]
