...
[grid="none"]
|=======================
| Hardware source files:  | astrarv32_imem.vhd       | processor-internal instruction memory (RAM or ROM)
|                         | astrarv32_imem_image.vhd | memory initialization image (VHDL package)
...
<<_linker_script>> (e.g. `USER_FLAGS += -Wl,--defsym,__astrarv32_rom_base=<IMEM_BASE>`).
...
file `rtl/core/astrarv32_imem_image.vhd`, which is automatically inserted into the IMEM (see
...
Alternatively, a custom IMEM can be connected via the <<_processor_external_bus_interface_xbus>>.
