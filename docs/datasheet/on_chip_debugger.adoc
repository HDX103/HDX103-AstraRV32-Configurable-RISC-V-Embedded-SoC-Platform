...
The ASTRARV32 Processor features an _on-chip debugger_ (OCD) compatible to the **Minimal RISC-V Debug Specification**
...
.Hands-On Tutorial
[TIP]
A simple example on how to use ASTRARV32 on-chip debugger in combination with OpenOCD and the GNU debugger is shown in
section https://stnolting.github.io/astrarv32/ug/#_debugging_using_the_on_chip_debugger[Debugging using the On-Chip Debugger]
of the User Guide.
...
.ASTRARV32 OCD Configuration Generics
[cols="<3,^2,^2,<8"]
...
.ASTRARV32 on-chip debugger complex
image::astrarv32_ocd_complex.png[align=center]

The ASTRARV32 on-chip debugger is based on five hardware modules:
...
openOCD is configured by a set of configuration scripts which are located in `astrarv32/sw/openocd`.
Two top-level scripts are provided: one for the single-core processor configuration (`astrarv32.cfg`)
and another one for the SMP dual-core configuration (`astrarv32.dual-core.cfg`).
Both scripts have the same format and include several helper scripts.

.ASTRARV32 openOCD single-core CFG file (`astrarv32.cfg`)
[source,tcl]
----
# configuration
set PATH [file dirname [file normalize [info script]]] <1>
set CORENAME astrarv32 <2>
set NUMCORES 1 <3>
...
<5> ASTRARV32-specific target configuration and initialization.
...
Semihosting is a mechanism developed by ARM that enables code running on the ASTRARV32 to communicate and use the
...
The ASTRARV32 software framework provides a build-in library for semihosting primitives (`sw/lib/include/astrarv32_semihosting.h`).
...
The debug transport module "DTM" (VHDL module: `rtl/core/astrarv32_debug_dtm.vhd`) provides a bridge between a standard 4-wire
...
.JTAG TAP Reset
[NOTE]
The ASTRARV32 JTAG TAP does not provide a dedicated reset signal ("TRST").
...
setups in [`astrarv32-setups`](https://github.com/stnolting/astrarv32-setups) for example implementations.
...
The debug module "DM" (VHDL module: `rtl/core/astrarv32_debug_dm.vhd`) acts as a translation interface between abstract
...
The ASTRARV32 DM follows the "Minimal RISC-V External Debug Specification" to provide full debugging capabilities while
...
.DM Spec. Version
[TIP]
The ASTRARV32 DM complies to the RISC-V DM spec version 1.0.
...
[NOTE]
The ASTRARV32 DM only supports **Access Register** abstract commands. These commands can only access the
...
The authenticator module is implemented as individual RTL module (`rtl/core/astrarv32_debug_auth.vhd`). By default, it implements
...
The ASTRARV32 CPU Debug Mode is compatible to the **Minimal RISC-V Debug Specification 1.0** `Sdext` (external debug)
...
complies to version v1.0 of that spec. Note that the ASTRARV32 trigger module only supports "type 6 - instruction address match"
...
| Reset value | `0x00000000`
| ISA         | `Zicsr` & `Sdtrig`
| Description | The features provided by this CSR are not implemented. Hence, this CSR is hardwired to all-zero.
|=======================
