...
The following table shows a summary of all available ASTRARV32 CSRs. The address field defines the CSR address for
the CSR access instructions. The "Name [ASM]" column provides the CSR name aliases that can be used in (inline) assembly.
The "Name [C]" column lists the name aliases that are defined by the ASTRARV32 core library. These can be used in plain C code.
...
.ASTRARV32 Control and Status Registers (CSRs)
[cols="<3,<5,<6,^3,<12"]
[options="header"]
|=======================
...
[NOTE]
The ASTRARV32 `misa` CSR is read-only. Hence, active CPU extensions are entirely defined by pre-synthesis configurations
...
| 23    | `CSR_MISA_X_EXT` | r/- | **X**: bit is always set to indicate non-standard / ASTRARV32-specific extensions
...
[TIP]
Machine-mode software can discover available `Z*` sub-extensions (like `Zicsr` or `Zfinx`) by checking the ASTRARV32-specific
<<_mxisah>> CSRs.
...
| 4:0  | r/w | **Exception code**: see <<_astrarv32_trap_listing>>
|=======================
...
|=======================
| Bit | Name [C] | R/W | Function
| 31:16 | `CSR_MIP_FIRQ15P` : `CSR_MIP_FIRQ0P` | r/- | **FIRQxP**: Fast interrupt channel 15..0 pending, see <<_astrarv32_specific_fast_interrupt_requests>>; cleared by source-specific mechanism
...
[TIP]
See section <<_astrarv32_specific_fast_interrupt_requests>> for the mapping of the FIRQ channels and the according
...
| ISA         | <<_zicsr_isa_extension,`Zicsr`>>
| Description | The `marchid` CSR is read-only and provides the ASTRARV32 official RISC-V open-source architecture ID
...
ASTRARV32 as BCD-coded number (example: `mimpid = 0x01020312` → 01.02.03.12 → version 1.2.3.12).
|=======================
...
// ####################################################################################################################
:sectnums:
==== ASTRARV32-Specific CSRs

.RISC-V-Compliant Mapping
[NOTE]
ASTRARV32-specific CSRs are mapped to addresses that are explicitly reserved for custom/implementation-specific use.
...
| Description | The `mxisa[h]` CSRs are ASTRARV32-specific read-only CSRs that help machine-mode software to
