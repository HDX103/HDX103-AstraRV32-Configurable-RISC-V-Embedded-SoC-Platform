<<<
:sectnums:
== ASTRARV32 Processor (SoC)

The ASTRARV32 Processor provides a RISC-V-based full-scale microcontroller-like SoC platform
build around the <<_astrarv32_central_processing_unit_cpu>>.

.The ASTRARV32 Processor (Block Diagram)
image::astrarv32_processor.png[align=center]
...
This section lists all configuration generics of the ASTRARV32 processor top entity (`rtl/astrarv32_top.vhd`).
...
.ASTRARV32 Processor Generic List; table abbreviations: the generic type "`suv(x:y)`" is an abbreviation for "`std_ulogic_vector(x downto y)`"
[cols="<3,^2,^2,<8"]
...
| `RISCV_ISA_Xcfu`      | boolean   | false         | Enable ASTRARV32-specific <<_xcfu_isa_extension>> (custom RISC-V instructions).
...
The following table shows the interface signals of the processor top entity (`rtl/core/astrarv32_top.vhd`).
...
.ASTRARV32 Processor Signal List
[cols="<3,^2,^2,^2,<8"]
[options="header",grid="rows"]
|=======================
...
Certain ASTRARV32 SoC modules are designed so that they can be substituted with a custom, external (Verilog) module.
...
GHDL-based <<_convert_to_verilog>> flow used by ASTRARV32.
...
| Bootloader ROM (<<_bootloader_rom_bootrom,BOOTROM>>)   | `rtl/core/astrarv32_bootrom.vhd` | `rtl/verilog/astrarv32_bootrom.v`
| Data memory (<<_data_memory_dmem,DMEM>>)               | `rtl/core/astrarv32_dmem.vhd`    | `rtl/verilog/astrarv32_dmem.v`   
| Instruction memory (<<_instruction_memory_imem,IMEM>>) | `rtl/core/astrarv32_imem.vhd`    | `rtl/verilog/astrarv32_imem.v`   
|=======================
...
the hardware, the processor implements a global "clock generator" (`astrarv32_sys.vhd`) that provides single-cycle _clock enables_
...
The ASTRARV32 processor includes a central reset sequencer (`astrarv32_sys.vhd`) that handles all reset requests
...
ensuring a valid reset of the entire processor (reset sequencer VHDL module: `astrarv32_sys_reset`). This system reset is
...
:sectnums:
==== ASTRARV32-Specific Fast Interrupt Requests

As part of the ASTRARV32-specific CPU extensions, the processor core features 16 fast interrupt request signals
...
.ASTRARV32 Fast Interrupt Request (FIRQ) Mapping
[cols="^3,<7"]
[options="header",grid="rows"]
|=======================
...
As a 32-bit architecture, ASTRARV32 can access up to 4GB of physical address space. The following figure shows
the **default** address space layout with the according memory capabilities (_physical memory attributes_ "PMAs"):

.ASTRARV32 Processor Address Space (Default Configuration)
...
for the ASTRARV32's internal memory-mapped peripherals and I/O modules and cannot be repurposed.
...
image::astrarv32_bus.png[1300]

.Bus System Infrastructure IP
[NOTE]
The individual components of the processor's bus system infrastructure are defined in `rtl/core/astrarv32_bus.vhd`.
...
The central bus gateway (VHDL component `astrarv32_bus_gateway`) distributes CPU accesses to the two main memory
...
this time windows is defined by a constant in the main ASTRARV32 package file. For **processor-external accesses**
(via the external bus interface) this timeout is defined by the `XBUS_TIMEOUT` top configuration generic.

.Internal Bus Timeout Configuration (package constant in `astrarv32_package.vhd`)
[source,vhdl]
----
constant int_bus_tmo_c : natural := 16; -- internal bus timeout window; has to be a power of two
----

.External Bus Timeout Configuration (top generic of `astrarv32_top.vhd`)
[source,vhdl]
----
...
The IO switch (VHDL component `astrarv32_bus_io_switch`) decodes accesses to the _procesor-internal IO/peripheral_
address space. This 2MB address space is used to map all ASTRARV32 peripherals (like UART, SPI, timers, etc.),
the bootloader ROM and the on-chip debugger. Each device occupies an address range of 64kB; so up to 32 individual
devices can be mapped. The according address map is defined in the main VHDL package file (`rtl/core/astrarv32_package.vhd`).
...
| `astrarv32_bus_amo_rmw` | <<_zaamo_isa_extension>>  | <<_atomic_read_modify_write_controller>>
| `astrarv32_bus_amo_rvs` | <<_zalrsc_isa_extension>> | <<_atomic_reservation_set_controller>>
|=======================
...
In ASTRARV32 a reservation is defined by a 64-byte address granule that provides a guarding mechanism to support
...
[WARNING]
Note that the reservation-set controller cannot track memory accesses outside of the ASTRARV32 processor
...
Depending on the configuration, the ASTRARV32 memory hierarchy provides several levels/layers of memory.
...
ASTRARV32 features up to three bus hosts: up to two CPU cores and the <<_direct_memory_access_controller_dma>>.
...
is no snooping implemented. ASTRARV32 supports two RISC-V memory ordering instructions that are used for
...
[IMPORTANT]
The ASTRARV32-specific implementation of the `fence[.i]` ordering instructions only provides a rather **weak**
...
The ASTRARV32 processor provides pre-defined boot configurations to adjust system start-up to
...
This ROM contains the executable image (`rtl/core/astrarv32_bootrom_image.vhd`) of the default ASTRARV32 <<_bootloader>>
...
initialized during synthesis with the according application image file (`rtl/core/astrarv32_imem_image.vhd`).
