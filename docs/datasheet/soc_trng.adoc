...
[grid="none"]
|=======================
| Hardware source files:  | astrarv32_trng.vhd    |
| Software driver files:  | astrarv32_trng.c      | link:https://stnolting.github.io/astrarv32/sw/astrarv32__trng_8c.html[Online software reference (Doxygen)]
|                         | astrarv32_trng.h      | link:https://stnolting.github.io/astrarv32/sw/astrarv32__trng_8h.html[Online software reference (Doxygen)]
...
|                         | `IO_TRNG_NUM_RO`    | total number of ring-oscillators
|                         | `IO_TRNG_NUM_INV`   | number of inverters in first ring-oscillator, has to be odd
|                         | `IO_TRNG_NUM_RBIT`  | number of raw bits to process for one output byte, has to be power of two
...
The ASTRARV32 true random number generator provides _physically_ true random numbers. It is based on free-running
...
[start=1]
. `IO_TRNG_NUM_RO` defines the total number of ring-oscillators.
. `IO_TRNG_NUM_INV` defines the number of inverters in the first ring-oscillator.
Each subsequent RO is two inverters longer than the previous one.
. `IO_TRNG_NUM_RBIT` defines the number of raw entropy bits obtained from the ring-oscillators that are scrmabled
and combined to produce a single random output byte.
...
.TRNG register map (`struct ASTRARV32_TRNG`)
[cols="<2,<1,<4,^1,<7"]
[options="header",grid="all"]
|=======================
| Address | Name [C] | Bit(s), Name [C] | R/W | Function
.8+<| `0xfffa0000` .8+<| `CTRL` <|`0`     `TRNG_CTRL_EN`                            ^| r/w <| TRNG enable
                                <|`1`     `TRNG_CTRL_FIFO_CLR`                      ^| -/w <| flush random data FIFO when set; auto-clears
                                <|`2`     `TRNG_CTRL_SIM_MODE`                      ^| r/- <| simulation mode (PRNG!)
                                <|`3`     `TRNG_CTRL_AVAIL`                         ^| r/- <| random data available when set
                                <|`7:4`   `TRNG_CTRL_FIFO_MSB : TRNG_CTRL_FIFO_LSB` ^| r/- <| FIFO depth (`log2(IO_TRNG_FIFO)`)
                                <|`11:8`  `TRNG_CTRL_NBIT_MSB : TRNG_CTRL_NBIT_LSB` ^| r/- <| FIFO depth (`log2(IO_TRNG_NUM_RBIT)`)
                                <|`19:12` `TRNG_CTRL_NRO_MSB : TRNG_CTRL_NRO_LSB`   ^| r/- <| Total number of ring-oscillators (`IO_TRNG_NUM_RO`)
                                <|`31:20` `TRNG_CTRL_NINV_MSB : TRNG_CTRL_NINV_LSB` ^| r/- <| Number of inverters in first ring-oscillator (`IO_TRNG_NUM_INV`)
