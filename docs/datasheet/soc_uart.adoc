...
[grid="none"]
|=======================
| Hardware source files:  | astrarv32_uart.vhd   |
| Software driver files:  | astrarv32_uart.c     | link:https://stnolting.github.io/astrarv32/sw/astrarv32__uart_8c.html[Online software reference (Doxygen)]
|                         | astrarv32_uart.h     | link:https://stnolting.github.io/astrarv32/sw/astrarv32__uart_8h.html[Online software reference (Doxygen)]
...
The ASTRARV32 UART provides a a standard universal asynchronous serial interface with independent transmitter and
...
.Standard Console
[NOTE]
All default example programs and software libraries of the ASTRARV32 software framework (including the bootloader
...
The ASTRARV32 UART supports optional hardware flow control using the standard CTS `uart0_ctsn_i` ("clear to send") and
...
.UART0 register map (`struct ASTRARV32_UART0`)
[cols="<4,<2,<5,^2,<5"]
...
.16+<| `0xfff50000` .16+<| `CTRL` <|`0`     `UART_CTRL_EN`                            ^| r/w <| UART enable
                                  ...
                                  <|`30`    `UART_CTRL_RX_OVER`                       ^| r/- <| RX FIFO overflow; cleared on read access
...
.UART1 register map (`struct ASTRARV32_UART1`)
[cols="<2,<1,<1,^1,<2"]
[options="header",grid="all"]
|=======================
| Address | Name [C] | Bit(s), Name [C] | R/W | Function
