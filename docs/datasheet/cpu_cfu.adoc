...
The Custom Functions Unit (CFU) is a ASTRARV32-specific ISA extension (<<_xcfu_isa_extension, `Xcfu`>>)
...
ASTRARV32-specific hardware extension/customization options is provided in the user guide section
https://stnolting.github.io/astrarv32/ug/#_adding_custom_hardware_modules[Adding Custom Hardware Modules].

.Default CFU Hardware/Software Example
[TIP]
The default CFU module (`rtl/core/astrarv32_cpu_alu_cfu.vhd`) implements the _Extended Tiny Encryption Algorithm (XTEA)_
...
ISA extensions. Note that the support/handling of the latter two OPCODES in ASTRARV32 might change in future.

.Available CFU OPCODES (defined in `sw/lib/include/astrarv32_intrinsics.h`)
[source,c]
----
...
original function call. For now, the ASTRARV32 software framework provides two pre-defined prototypes for custom
instructions:

.RISC-V Instruction Prototypes (simplified; defined in `sw/lib/include/astrarv32_intrinsics.h`)
[source,c]
----
...
The CFU hardware module (`rtl/core/astrarv32_cpu_alu_cfu.vhd`) is tightly integrated into to CPU and coupled as ALU
