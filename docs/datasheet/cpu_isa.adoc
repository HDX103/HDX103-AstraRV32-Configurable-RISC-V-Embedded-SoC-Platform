...
The ASTRARV32 CPU provides several optional RISC-V-compliant and custom/user-defined ISA extensions.
...
.ASTRARV32 Instruction Set Extensions
[cols="<2,<5,<3"]
...
| <<_x_isa_extension,`X`>>                 | Platform-specific / ASTRARV32-specific extension               | Always enabled
...
[NOTE]
The ASTRARV32 instruction-decoding logic follows a strict illegal instruction detection scheme:
...
Note that the ASTRARV32 `C` ISA extension only includes the `Zca` instructions; i.e. all instructions from `C`
...
This ISA extension is implemented as multi-cycle ALU co-process (`rtl/core/astrarv32_cpu_alu_muldiv.vhd`).
...
The ASTRARV32-specific ISA extensions `X` is always enabled. The most important points of the ASTRARV32-specific extensions are:
...
* Additional <<_astrarv32_specific_csrs>>.
...
This ISA extension is implemented as multi-cycle ALU co-process (`rtl/core/astrarv32_cpu_alu_fpu.vhd`).
...
.Subnormal Numbers
[WARNING]
Subnormal numbers ("de-normalized" numbers, i.e. exponent = 0) are not supported by the ASTRARV32 FPU.
...
This ISA extension is implemented as multi-cycle ALU co-processor (`rtl/core/astrarv32_cpu_alu_cond.vhd`).
...
In additions to the base counters the ASTRARV32 CPU provides up to 29 hardware performance monitors (HPM; <<_mhpmcounterh>>
...
perform some other system/environment-class actions. ASTRARV32 can implement this ISA extension for software compatibility.
...
ALU co-processor (`rtl/core/astrarv32_cpu_alu_bitmanip.vhd`).
...
generic. This ISA extension is implemented as multi-cycle ALU co-processor (`rtl/core/astrarv32_cpu_alu_bitmanip.vhd`).
...
This ISA extension is implemented as multi-cycle ALU co-processor (`rtl/core/astrarv32_cpu_alu_bitmanip.vhd`).
...
This ISA extension is implemented as multi-cycle ALU co-processor (`rtl/core/astrarv32_cpu_alu_crypto.vhd`).
...
The `Xcfu` presents a custom ASTRARV32-specific ISA extension. It adds the <<_custom_functions_unit_cfu>> to
...
The ASTRARV32 physical memory protection (PMP) provides an elementary memory protection mechanism that can be used
...
permissions. The ASTRARV32 PMP is fully compatible to the RISC-V Privileged Architecture Specifications and is
