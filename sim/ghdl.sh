...
find ../rtl/core ../sim -type f -name '*.vhd'  -exec \
  ghdl -i --std=08 --workdir=build --ieee=standard --work=astrarv32 {} \;

# GHDL analyze
$GHDL -m --work=astrarv32 --workdir=build --std=08 astrarv32_tb
...
# prepare GHDL run
runcmd="$GHDL -r --work=astrarv32 --workdir=build --std=08 astrarv32_tb \
  --max-stack-alloc=0 \
