-- ================================================================================ --
-- ASTRARV32 SoC - RISC-V Core-Local Interruptor (CLINT)                              --
-- -------------------------------------------------------------------------------- --
...
architecture astrarv32_clint_rtl of astrarv32_clint is

  -- timer interrupt generator --
  component astrarv32_clint_mtimecmp
  port (
    clk_i   : in  std_ulogic;
...
begin
  ...
  -- -------------------------------------------------------------------------------------------
  astrarv32_clint_mtime_inst: entity astrarv32.astrarv32_prim_cnt
  generic map (
    CWIDTH => 64
  )
  port map (
  ...
  -- -------------------------------------------------------------------------------------------
  astrarv32_clint_mtimecmp_gen:
  for i in 0 to NUM_HARTS-1 generate

    astrarv32_clint_mtimecmp_inst: astrarv32_clint_mtimecmp
    port map (
      clk_i   => clk_i,
      rstn_i  => rstn_i,
      mtime_i => mtime,
...
-- ================================================================================ --
-- ASTRARV32 SoC - CLINT MTIMECMP (per-hart time comparator)                          --
-- -------------------------------------------------------------------------------- --
-- The ASTRARV32 RISC-V Processor - https://github.com/stnolting/astrarv32              --
-- Copyright (c) ASTRARV32 contributors.                                              --
-- Copyright (c) 2020 - 2026 Stephan Nolting. All rights reserved.                  --
...
entity astrarv32_clint_mtimecmp is
  port (
    clk_i   : in  std_ulogic; -- global clock line
    rstn_i  : in  std_ulogic; -- global reset line, low-active, async
...
architecture astrarv32_clint_mtimecmp_rtl of astrarv32_clint_mtimecmp is
