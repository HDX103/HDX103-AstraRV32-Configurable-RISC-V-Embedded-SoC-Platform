-- ================================================================================ --
-- ASTRARV32 SoC - Instruction Memory (IMEM)                                          --
-- -------------------------------------------------------------------------------- --
...
architecture astrarv32_imem_rtl of astrarv32_imem is

  component astrarv32_imem_rom -- IMEM ROM wrapper
  generic (
    AWIDTH : natural;
    OUTREG : boolean
  );
...
begin
  ...
  imem_rom:
  if INITROM generate
    imem_rom_inst: astrarv32_imem_rom
    generic map (
      AWIDTH => AWIDTH,
      OUTREG => OUTREG
    )
  ...
  if not INITROM generate
    ...
    for i in 0 to 3 generate -- 4x byte-wide RAMs
      imem_ram_inst: entity astrarv32.astrarv32_prim_spram
      generic map (
        AWIDTH => AWIDTH-2,
        DWIDTH => 8,
        OUTREG => OUTREG
...
-- ================================================================================ --
-- ASTRARV32 SoC - Instruction Memory (IMEM) - ROM Primitive Wrapper                  --
-- ================================================================================ --
...
library astrarv32;
use astrarv32.astrarv32_package.all;
use astrarv32.astrarv32_imem_image.all;

entity astrarv32_imem_rom is
  generic (
...
architecture astrarv32_imem_rom_rtl of astrarv32_imem_rom is
...
begin
  ...
  assert (image_size_c <= 2**AWIDTH) report
    "[ASTRARV32] IMEM image (" & natural'image(image_size_c) & " bytes) " &
