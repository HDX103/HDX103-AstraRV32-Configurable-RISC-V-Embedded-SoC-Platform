// ================================================================================ //
// ASTRARV32 - Verilog testbench                                                      //
// -------------------------------------------------------------------------------- //
// Simple testbench for the auto-generated all-Verilog version of ASTRARV32.          //
// Checks for the initial UART output of the bootloader ("ASTRARV32").                //
// -------------------------------------------------------------------------------- //
...
`ifdef DUMP_WAVE
    ...
    $dumpvars();
`endif
    $display ("[TB] ASTRARV32 Verilog testbench\n");
    clk = 0;
    nrst = 0;
  ...
  // in the ASTRARV32 VHDL wrapper *before* synthesizing the generated Verilog code
  astrarv32_verilog_wrapper astrarv32_verilog_inst (
    .clk_i       (clk),
    .rstn_i      (nrst),
    .uart0_rxd_i (1'b0),
  ...
  uart_sim_receiver #(
    ...
    .BAUD_RATE  (19200)      // default baud rate of the ASTRARV32 bootloader
  ...
  always @(posedge clk or negedge nrst) begin
    if (nrst == 1'b0) begin
      ...
      // check for result string: "ASTRARV32" is sent by the default bootloader right after reset
      if ((uart_log[0] == "N") &&
          (uart_log[1] == "E") &&
          (uart_log[2] == "O") &&
          (uart_log[3] == "R") &&
...
// https://github.com/stnolting/astrarv32-verilog
